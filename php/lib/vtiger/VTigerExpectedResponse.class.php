 ...
 * @package entity
 */
class VTigerExpectedResponse extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerExpectedResponse';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_expectedresponse';
	const SQL_INSERT='INSERT INTO `vtiger_expectedresponse` (`expectedresponseid`,`expectedresponse`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_expectedresponse` (`expectedresponse`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_expectedresponse` SET `expectedresponseid`=?,`expectedresponse`=?,`presence`=?,`picklist_valueid`=? WHERE `expectedresponseid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_expectedresponse` WHERE `expectedresponseid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_expectedresponse` WHERE `expectedresponseid`=?';
	const FIELD_EXPECTEDRESPONSEID=-1582158925;
	const FIELD_EXPECTEDRESPONSE=1415111832;
	const FIELD_PRESENCE=1571777946;
	const FIELD_PICKLIST_VALUEID=250476523;
	private static $PRIMARY_KEYS=array(self::FIELD_EXPECTEDRESPONSEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_EXPECTEDRESPONSEID);
	private static $FIELD_NAMES=array(
		self::FIELD_EXPECTEDRESPONSEID=>'expectedresponseid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_EXPECTEDRESPONSEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_EXPECTEDRESPONSEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_EXPECTEDRESPONSE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_EXPECTEDRESPONSEID=>null,
		self::FIELD_EXPECTEDRESPONSE=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $expectedResponseId;
	private $expectedResponse;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for expectedresponseid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $expectedResponseId
	 * @return VTigerExpectedResponse
	 */
	public function &setExpectedResponseId($expectedResponseId) {
		$this->notifyChanged(self::FIELD_EXPECTEDRESPONSEID,$this->expectedResponseId,$expectedResponseId);
		$this->expectedResponseId=$expectedResponseId;
		return $this;
	}

	/**
	 * get value for expectedresponseid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getExpectedResponseId() {
	...
	public function toArray() {
		return array(
			self::FIELD_EXPECTEDRESPONSEID=>$this->getExpectedResponseId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerExpectedResponse instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerExpectedResponse $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerExpectedResponse[]
	 */
	public static function findByExample(PDO $db,VTigerExpectedResponse $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerExpectedResponse instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_expectedresponse`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerExpectedResponse instances
	 *
	...
	/**
	 * returns the result as an array of VTigerExpectedResponse instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerExpectedResponse[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerExpectedResponse instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_expectedresponse`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setExpectedResponseId($result['expectedresponseid']);
	 ...
	 * @return VTigerExpectedResponse
	 */
	public static function findById(PDO $db,$expectedResponseId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$expectedResponseId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getExpectedResponseId());
		$stmt->bindValue(2,$this->getExpectedResponse());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getExpectedResponseId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getExpectedResponse());
		...
		if (false!==$lastInsertId) {
			$this->setExpectedResponseId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getExpectedResponseId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerExpectedResponse';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerExpectedResponse');
	}

	/**
	 * get single VTigerExpectedResponse instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerExpectedResponse from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerExpectedResponse') as $node) {
