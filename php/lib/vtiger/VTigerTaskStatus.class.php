 ...
 * @package entity
 */
class VTigerTaskStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerTaskStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_taskstatus';
	const SQL_INSERT='INSERT INTO `vtiger_taskstatus` (`taskstatusid`,`taskstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_taskstatus` (`taskstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_taskstatus` SET `taskstatusid`=?,`taskstatus`=?,`presence`=?,`picklist_valueid`=? WHERE `taskstatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_taskstatus` WHERE `taskstatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_taskstatus` WHERE `taskstatusid`=?';
	const FIELD_TASKSTATUSID=-1788908621;
	const FIELD_TASKSTATUS=-766106472;
	const FIELD_PRESENCE=2135095324;
	const FIELD_PICKLIST_VALUEID=471326829;
	private static $PRIMARY_KEYS=array(self::FIELD_TASKSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_TASKSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_TASKSTATUSID=>'taskstatusid',
		self::FIELD_TASKSTATUS=>'taskstatus',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_TASKSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
		self::FIELD_TASKSTATUS=>Db2PhpEntity::PHP_TYPE_STRING,
		self::FIELD_PRESENCE=>Db2PhpEntity::PHP_TYPE_INT,
		self::FIELD_PICKLIST_VALUEID=>Db2PhpEntity::PHP_TYPE_INT);
	private static $FIELD_TYPES=array(
		self::FIELD_TASKSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_TASKSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,true),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_TASKSTATUSID=>null,
		self::FIELD_TASKSTATUS=>null,
	...
	/**
	 * set value for taskstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $taskStatusId
	 * @return VTigerTaskStatus
	 */
	public function &setTaskStatusId($taskStatusId) {
		$this->notifyChanged(self::FIELD_TASKSTATUSID,$this->taskStatusId,$taskStatusId);
		$this->taskStatusId=$taskStatusId;
		return $this;
	}

	/**
	 * get value for taskstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function &setTaskStatus($taskStatus) {
		$this->notifyChanged(self::FIELD_TASKSTATUS,$this->taskStatus,$taskStatus);
	...
	public function toArray() {
		return array(
			self::FIELD_TASKSTATUSID=>$this->getTaskStatusId(),
			self::FIELD_TASKSTATUS=>$this->getTaskStatus(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerTaskStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerTaskStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerTaskStatus[]
	 */
	public static function findByExample(PDO $db,VTigerTaskStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerTaskStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_taskstatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerTaskStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerTaskStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerTaskStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerTaskStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_taskstatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setTaskStatusId($result['taskstatusid']);
		$this->setTaskStatus($result['taskstatus']);
	 ...
	 * @return VTigerTaskStatus
	 */
	public static function findById(PDO $db,$taskStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$taskStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getTaskStatusId());
		$stmt->bindValue(2,$this->getTaskStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getTaskStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getTaskStatus());
		...
		if (false!==$lastInsertId) {
			$this->setTaskStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getTaskStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerTaskStatus');
	}

	/**
	 * get single VTigerTaskStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerTaskStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerTaskStatus') as $node) {
