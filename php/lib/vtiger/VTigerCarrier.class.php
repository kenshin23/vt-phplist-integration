 ...
 * @package entity
 */
class VTigerCarrier extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerCarrier';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_carrier';
	const SQL_INSERT='INSERT INTO `vtiger_carrier` (`carrierid`,`carrier`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_carrier` (`carrier`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_carrier` SET `carrierid`=?,`carrier`=?,`presence`=?,`picklist_valueid`=? WHERE `carrierid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_carrier` WHERE `carrierid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_carrier` WHERE `carrierid`=?';
	const FIELD_CARRIERID=1284345319;
	const FIELD_CARRIER=-481344564;
	const FIELD_PRESENCE=976212647;
	const FIELD_PICKLIST_VALUEID=-223150088;
	private static $PRIMARY_KEYS=array(self::FIELD_CARRIERID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_CARRIERID);
	...
	private static $FIELD_TYPES=array(
		self::FIELD_CARRIERID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_CARRIER=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerCarrier instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerCarrier $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerCarrier[]
	 */
	public static function findByExample(PDO $db,VTigerCarrier $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerCarrier instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_carrier`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerCarrier instances
	 *
	...
	/**
	 * returns the result as an array of VTigerCarrier instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerCarrier[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerCarrier instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_carrier`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return VTigerCarrier
	 */
	public static function findById(PDO $db,$carrierId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$carrierId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getCarrierId());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getCarrierId()) {
			...
			$stmt->bindValue(1,$this->getCarrier());
		...
		if (false!==$lastInsertId) {
			$this->setCarrierId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getCarrierId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerCarrier');
	}

	/**
	 * get single VTigerCarrier instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerCarrier from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerCarrier') as $node) {
