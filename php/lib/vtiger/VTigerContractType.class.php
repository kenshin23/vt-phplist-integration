 ...
 * @package entity
 */
class VTigerContractType extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerContractType';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_contract_type';
	const SQL_INSERT='INSERT INTO `vtiger_contract_type` (`contract_typeid`,`contract_type`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_contract_type` (`contract_type`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_contract_type` SET `contract_typeid`=?,`contract_type`=?,`presence`=?,`picklist_valueid`=? WHERE `contract_typeid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_contract_type` WHERE `contract_typeid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_contract_type` WHERE `contract_typeid`=?';
	const FIELD_CONTRACT_TYPEID=-1735118779;
	const FIELD_CONTRACT_TYPE=1790371242;
	const FIELD_PRESENCE=-130877512;
	const FIELD_PICKLIST_VALUEID=-1169390583;
	private static $PRIMARY_KEYS=array(self::FIELD_CONTRACT_TYPEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_CONTRACT_TYPEID);
	private static $FIELD_NAMES=array(
		self::FIELD_CONTRACT_TYPEID=>'contract_typeid',
		self::FIELD_CONTRACT_TYPE=>'contract_type',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'picklist_valueid');
	private static $PROPERTY_NAMES=array(
		self::FIELD_CONTRACT_TYPEID=>'contractTypeId',
		self::FIELD_CONTRACT_TYPE=>'contractType',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'pickListValueId');
	private static $PROPERTY_TYPES=array(
		self::FIELD_CONTRACT_TYPEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_CONTRACT_TYPEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_CONTRACT_TYPE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_CONTRACT_TYPEID=>null,
		self::FIELD_CONTRACT_TYPE=>'',
	...
	/**
	 * set value for contract_typeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $contractTypeId
	 * @return VTigerContractType
	 */
	public function &setContractTypeId($contractTypeId) {
		$this->notifyChanged(self::FIELD_CONTRACT_TYPEID,$this->contractTypeId,$contractTypeId);
		$this->contractTypeId=$contractTypeId;
		return $this;
	}

	/**
	 * get value for contract_typeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_CONTRACT_TYPEID=>$this->getContractTypeId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerContractType instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerContractType $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerContractType[]
	 */
	public static function findByExample(PDO $db,VTigerContractType $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerContractType instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerContractType instances
	 *
	...
	/**
	 * returns the result as an array of VTigerContractType instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerContractType[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerContractType instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_contract_type`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setContractTypeId($result['contract_typeid']);
	 ...
	 * @return VTigerContractType
	 */
	public static function findById(PDO $db,$contractTypeId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$contractTypeId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getContractTypeId());
		$stmt->bindValue(2,$this->getContractType());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getContractTypeId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getContractType());
		...
		if (false!==$lastInsertId) {
			$this->setContractTypeId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getContractTypeId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerContractType');
	}

	/**
	 * get single VTigerContractType instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerContractType from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerContractType') as $node) {
