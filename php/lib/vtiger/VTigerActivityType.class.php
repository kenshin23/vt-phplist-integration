 ...
 * @package entity
 */
class VTigerActivityType extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerActivityType';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_activitytype';
	const SQL_INSERT='INSERT INTO `vtiger_activitytype` (`activitytypeid`,`activitytype`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_activitytype` (`activitytype`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_activitytype` SET `activitytypeid`=?,`activitytype`=?,`presence`=?,`picklist_valueid`=? WHERE `activitytypeid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_activitytype` WHERE `activitytypeid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_activitytype` WHERE `activitytypeid`=?';
	const FIELD_ACTIVITYTYPEID=-218883533;
	const FIELD_ACTIVITYTYPE=1917088536;
	const FIELD_PRESENCE=931278026;
	const FIELD_PICKLIST_VALUEID=-765032677;
	private static $PRIMARY_KEYS=array(self::FIELD_ACTIVITYTYPEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_ACTIVITYTYPEID);
	private static $FIELD_NAMES=array(
		self::FIELD_ACTIVITYTYPEID=>'activitytypeid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_ACTIVITYTYPEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_ACTIVITYTYPEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_ACTIVITYTYPEID=>null,
	...
	/**
	 * set value for activitytypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $activityTypeId
	 * @return VTigerActivityType
	 */
	public function &setActivityTypeId($activityTypeId) {
		$this->notifyChanged(self::FIELD_ACTIVITYTYPEID,$this->activityTypeId,$activityTypeId);
		$this->activityTypeId=$activityTypeId;
		return $this;
	}

	/**
	 * get value for activitytypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_ACTIVITYTYPEID=>$this->getActivityTypeId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerActivityType instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerActivityType $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerActivityType[]
	 */
	public static function findByExample(PDO $db,VTigerActivityType $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerActivityType instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_activitytype`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerActivityType instances
	 *
	...
	/**
	 * returns the result as an array of VTigerActivityType instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerActivityType[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerActivityType instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_activitytype`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setActivityTypeId($result['activitytypeid']);
	 ...
	 * @return VTigerActivityType
	 */
	public static function findById(PDO $db,$activityTypeId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$activityTypeId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getActivityTypeId());
		$stmt->bindValue(2,$this->getActivityType());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getActivityTypeId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getActivityType());
		...
		if (false!==$lastInsertId) {
			$this->setActivityTypeId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getActivityTypeId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerActivityType');
	}

	/**
	 * get single VTigerActivityType instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerActivityType from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerActivityType') as $node) {
