 ...
 * @package entity
 */
class VTigerEventStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerEventStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_eventstatus';
	const SQL_INSERT='INSERT INTO `vtiger_eventstatus` (`eventstatusid`,`eventstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_eventstatus` (`eventstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_eventstatus` SET `eventstatusid`=?,`eventstatus`=?,`presence`=?,`picklist_valueid`=? WHERE `eventstatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_eventstatus` WHERE `eventstatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_eventstatus` WHERE `eventstatusid`=?';
	const FIELD_EVENTSTATUSID=-1590102257;
	const FIELD_EVENTSTATUS=-91040012;
	const FIELD_PRESENCE=136017427;
	const FIELD_PICKLIST_VALUEID=-987072668;
	private static $PRIMARY_KEYS=array(self::FIELD_EVENTSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_EVENTSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_EVENTSTATUSID=>'eventstatusid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_EVENTSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_EVENTSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_EVENTSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_EVENTSTATUSID=>null,
		self::FIELD_EVENTSTATUS=>'',
	...
	/**
	 * set value for eventstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $eventStatusId
	 * @return VTigerEventStatus
	 */
	public function &setEventStatusId($eventStatusId) {
		$this->notifyChanged(self::FIELD_EVENTSTATUSID,$this->eventStatusId,$eventStatusId);
		$this->eventStatusId=$eventStatusId;
		return $this;
	}

	/**
	 * get value for eventstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_EVENTSTATUSID=>$this->getEventStatusId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerEventStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerEventStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerEventStatus[]
	 */
	public static function findByExample(PDO $db,VTigerEventStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerEventStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_eventstatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerEventStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerEventStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerEventStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerEventStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_eventstatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setEventStatusId($result['eventstatusid']);
	 ...
	 * @return VTigerEventStatus
	 */
	public static function findById(PDO $db,$eventStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$eventStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getEventStatusId());
		$stmt->bindValue(2,$this->getEventStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getEventStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getEventStatus());
		...
		if (false!==$lastInsertId) {
			$this->setEventStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getEventStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerEventStatus');
	}

	/**
	 * get single VTigerEventStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerEventStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerEventStatus') as $node) {
