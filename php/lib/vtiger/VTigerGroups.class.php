 ...
 * @package entity
 */
class VTigerGroups extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerGroups';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_groups';
	const SQL_INSERT='INSERT INTO `vtiger_groups` (`groupid`,`groupname`,`description`) VALUES (?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_groups` (`groupid`,`groupname`,`description`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_groups` SET `groupid`=?,`groupname`=?,`description`=? WHERE `groupid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_groups` WHERE `groupid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_groups` WHERE `groupid`=?';
	const FIELD_GROUPID=-2084280650;
	const FIELD_GROUPNAME=-1538795162;
	const FIELD_DESCRIPTION=-257573672;
	private static $PRIMARY_KEYS=array(self::FIELD_GROUPID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerGroups instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerGroups $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerGroups[]
	 */
	public static function findByExample(PDO $db,VTigerGroups $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerGroups instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerGroups instances
	 *
	...
	/**
	 * returns the result as an array of VTigerGroups instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerGroups[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerGroups instances.
	 *
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(4,$this->getGroupId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerDaTashaReGRp2gRp's which this VTigerGroups references.
	 * `vtiger_groups`.`groupid` -> `vtiger_datashare_grp2grp`.`to_groupid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return VTigerDaTashaReGRp2gRp[]
	 */
	public function fetchVTigerDaTashaReGRp2gRpCollection(PDO $db, $sort=null) {
		$filter=array(VTigerDaTashaReGRp2gRp::FIELD_TO_GROUPID=>$this->getGroupId());
		return VTigerDaTashaReGRp2gRp::findByFilter($db, $filter, true, $sort);
	}

	/**
	 * Fetch VTigerGroup2GroupRel's which this VTigerGroups references.
	 * `vtiger_groups`.`groupid` -> `vtiger_group2grouprel`.`groupid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return VTigerGroup2GroupRel[]
	 */
	public function fetchVTigerGroup2GroupRelCollection(PDO $db, $sort=null) {
		$filter=array(VTigerGroup2GroupRel::FIELD_GROUPID=>$this->getGroupId());
		return VTigerGroup2GroupRel::findByFilter($db, $filter, true, $sort);
	}
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerGroups');
	}

	/**
	 * get single VTigerGroups instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerGroups from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerGroups') as $node) {
