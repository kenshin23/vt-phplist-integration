 ...
 * @package entity
 */
class VTigerPriceBookCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerPriceBookCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_pricebookcf';
	const SQL_INSERT='INSERT INTO `vtiger_pricebookcf` (`pricebookid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_pricebookcf` (`pricebookid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_pricebookcf` SET `pricebookid`=? WHERE `pricebookid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_pricebookcf` WHERE `pricebookid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_pricebookcf` WHERE `pricebookid`=?';
	const FIELD_PRICEBOOKID=140940094;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerPriceBookCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerPriceBookCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerPriceBookCf[]
	 */
	public static function findByExample(PDO $db,VTigerPriceBookCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerPriceBookCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_pricebookcf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerPriceBookCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerPriceBookCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerPriceBookCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerPriceBookCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_pricebookcf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getPriceBookId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerPriceBook which references this VTigerPriceBookCf. Will return null in case reference is invalid.
	 * `vtiger_pricebookcf`.`pricebookid` -> `vtiger_pricebook`.`pricebookid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerPriceBookCf');
	}

	/**
	 * get single VTigerPriceBookCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerPriceBookCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerPriceBookCf') as $node) {
