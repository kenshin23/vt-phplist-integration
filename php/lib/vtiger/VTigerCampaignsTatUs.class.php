 ...
 * @package entity
 */
class VTigerCampaignsTatUs extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerCampaignsTatUs';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_campaignstatus';
	const SQL_INSERT='INSERT INTO `vtiger_campaignstatus` (`campaignstatusid`,`campaignstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_campaignstatus` (`campaignstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_campaignstatus` SET `campaignstatusid`=?,`campaignstatus`=?,`presence`=?,`picklist_valueid`=? WHERE `campaignstatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_campaignstatus` WHERE `campaignstatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_campaignstatus` WHERE `campaignstatusid`=?';
	const FIELD_CAMPAIGNSTATUSID=-1514916141;
	const FIELD_CAMPAIGNSTATUS=-2120009800;
	const FIELD_PRESENCE=-99168207;
	const FIELD_PICKLIST_VALUEID=-80686206;
	private static $PRIMARY_KEYS=array(self::FIELD_CAMPAIGNSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_CAMPAIGNSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_CAMPAIGNSTATUSID=>'campaignstatusid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_CAMPAIGNSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_CAMPAIGNSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_CAMPAIGNSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_CAMPAIGNSTATUSID=>null,
		self::FIELD_CAMPAIGNSTATUS=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $campaignsTatUSid;
	private $campaignsTatUs;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for campaignstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $campaignsTatUSid
	 * @return VTigerCampaignsTatUs
	 */
	public function &setCampaignsTatUSid($campaignsTatUSid) {
		$this->notifyChanged(self::FIELD_CAMPAIGNSTATUSID,$this->campaignsTatUSid,$campaignsTatUSid);
		$this->campaignsTatUSid=$campaignsTatUSid;
		return $this;
	}

	/**
	 * get value for campaignstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_CAMPAIGNSTATUSID=>$this->getCampaignsTatUSid(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerCampaignsTatUs instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerCampaignsTatUs $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerCampaignsTatUs[]
	 */
	public static function findByExample(PDO $db,VTigerCampaignsTatUs $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerCampaignsTatUs instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_campaignstatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerCampaignsTatUs instances
	 *
	...
	/**
	 * returns the result as an array of VTigerCampaignsTatUs instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerCampaignsTatUs[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerCampaignsTatUs instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_campaignstatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setCampaignsTatUSid($result['campaignstatusid']);
	 ...
	 * @return VTigerCampaignsTatUs
	 */
	public static function findById(PDO $db,$campaignsTatUSid) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$campaignsTatUSid);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getCampaignsTatUSid());
		$stmt->bindValue(2,$this->getCampaignsTatUs());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getCampaignsTatUSid()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getCampaignsTatUs());
		...
		if (false!==$lastInsertId) {
			$this->setCampaignsTatUSid($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getCampaignsTatUSid());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerCampaignsTatUs';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerCampaignsTatUs');
	}

	/**
	 * get single VTigerCampaignsTatUs instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerCampaignsTatUs from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerCampaignsTatUs') as $node) {
