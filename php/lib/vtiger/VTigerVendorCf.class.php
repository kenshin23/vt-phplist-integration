 ...
 * @package entity
 */
class VTigerVendorCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerVendorCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_vendorcf';
	const SQL_INSERT='INSERT INTO `vtiger_vendorcf` (`vendorid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_vendorcf` (`vendorid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_vendorcf` SET `vendorid`=? WHERE `vendorid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_vendorcf` WHERE `vendorid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_vendorcf` WHERE `vendorid`=?';
	const FIELD_VENDORID=-601160048;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerVendorCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerVendorCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerVendorCf[]
	 */
	public static function findByExample(PDO $db,VTigerVendorCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerVendorCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_vendorcf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerVendorCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerVendorCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerVendorCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerVendorCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_vendorcf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getVendorId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerVendor which references this VTigerVendorCf. Will return null in case reference is invalid.
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerVendorCf');
	}

	/**
	 * get single VTigerVendorCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerVendorCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerVendorCf') as $node) {
