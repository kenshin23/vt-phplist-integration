 ...
 * @package entity
 */
class VTigerCampaignsCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerCampaignsCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_campaignscf';
	const SQL_INSERT='INSERT INTO `vtiger_campaignscf` (`campaignid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_campaignscf` (`campaignid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_campaignscf` SET `campaignid`=? WHERE `campaignid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_campaignscf` WHERE `campaignid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_campaignscf` WHERE `campaignid`=?';
	const FIELD_CAMPAIGNID=-569974007;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerCampaignsCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerCampaignsCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerCampaignsCf[]
	 */
	public static function findByExample(PDO $db,VTigerCampaignsCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerCampaignsCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_campaignscf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerCampaignsCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerCampaignsCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerCampaignsCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerCampaignsCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_campaignscf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getCampaignId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerCampaign which references this VTigerCampaignsCf. Will return null in case reference is invalid.
	 * `vtiger_campaignscf`.`campaignid` -> `vtiger_campaign`.`campaignid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return VTigerCampaign
	 */
	public function fetchVTigerCampaign(PDO $db, $sort=null) {
		$filter=array(VTigerCampaign::FIELD_CAMPAIGNID=>$this->getCampaignId());
		$result=VTigerCampaign::findByFilter($db, $filter, true, $sort);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerCampaignsCf');
	}

	/**
	 * get single VTigerCampaignsCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerCampaignsCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerCampaignsCf') as $node) {
