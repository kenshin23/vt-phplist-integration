 ...
 * @package entity
 */
class VTigerGlAcct extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerGlAcct';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_glacct';
	const SQL_INSERT='INSERT INTO `vtiger_glacct` (`glacctid`,`glacct`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_glacct` (`glacct`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_glacct` SET `glacctid`=?,`glacct`=?,`presence`=?,`picklist_valueid`=? WHERE `glacctid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_glacct` WHERE `glacctid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_glacct` WHERE `glacctid`=?';
	const FIELD_GLACCTID=575501843;
	const FIELD_GLACCT=-1778170120;
	const FIELD_PRESENCE=240327035;
	const FIELD_PICKLIST_VALUEID=1927033548;
	private static $PRIMARY_KEYS=array(self::FIELD_GLACCTID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_GLACCTID);
	private static $FIELD_NAMES=array(
		self::FIELD_GLACCTID=>'glacctid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_GLACCTID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_GLACCTID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_GLACCT=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	/**
	 * set value for glacctid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $glAcctId
	 * @return VTigerGlAcct
	 */
	public function &setGlAcctId($glAcctId) {
		$this->notifyChanged(self::FIELD_GLACCTID,$this->glAcctId,$glAcctId);
		$this->glAcctId=$glAcctId;
		return $this;
	}

	/**
	 * get value for glacctid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getGlAcctId() {
	...
	public function toArray() {
		return array(
			self::FIELD_GLACCTID=>$this->getGlAcctId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerGlAcct instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerGlAcct $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerGlAcct[]
	 */
	public static function findByExample(PDO $db,VTigerGlAcct $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerGlAcct instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerGlAcct instances
	 *
	...
	/**
	 * returns the result as an array of VTigerGlAcct instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerGlAcct[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerGlAcct instances.
	 *
	...
	public function assignByHash($result) {
		$this->setGlAcctId($result['glacctid']);
	 ...
	 * @return VTigerGlAcct
	 */
	public static function findById(PDO $db,$glAcctId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$glAcctId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getGlAcctId());
		$stmt->bindValue(2,$this->getGlAcct());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getGlAcctId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getGlAcct());
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getGlAcctId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerGlAcct');
	}

	/**
	 * get single VTigerGlAcct instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerGlAcct from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerGlAcct') as $node) {
