 ...
 * @package entity
 */
class VTigerContractStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerContractStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_contract_status';
	const SQL_INSERT='INSERT INTO `vtiger_contract_status` (`contract_statusid`,`contract_status`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_contract_status` (`contract_status`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_contract_status` SET `contract_statusid`=?,`contract_status`=?,`presence`=?,`picklist_valueid`=? WHERE `contract_statusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_contract_status` WHERE `contract_statusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_contract_status` WHERE `contract_statusid`=?';
	const FIELD_CONTRACT_STATUSID=-536854539;
	const FIELD_CONTRACT_STATUS=-465362598;
	const FIELD_PRESENCE=1980336064;
	const FIELD_PICKLIST_VALUEID=582388753;
	private static $PRIMARY_KEYS=array(self::FIELD_CONTRACT_STATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_CONTRACT_STATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_CONTRACT_STATUSID=>'contract_statusid',
	...
	private static $PROPERTY_NAMES=array(
		self::FIELD_CONTRACT_STATUSID=>'contractStatusId',
		self::FIELD_CONTRACT_STATUS=>'contractStatus',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'pickListValueId');
	private static $PROPERTY_TYPES=array(
		self::FIELD_CONTRACT_STATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_CONTRACT_STATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_CONTRACT_STATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_CONTRACT_STATUSID=>null,
		self::FIELD_CONTRACT_STATUS=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $contractStatusId;
	private $contractStatus;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for contract_statusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $contractStatusId
	 * @return VTigerContractStatus
	 */
	public function &setContractStatusId($contractStatusId) {
		$this->notifyChanged(self::FIELD_CONTRACT_STATUSID,$this->contractStatusId,$contractStatusId);
		$this->contractStatusId=$contractStatusId;
		return $this;
	}

	/**
	 * get value for contract_statusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getContractStatusId() {
	...
	public function toArray() {
		return array(
			self::FIELD_CONTRACT_STATUSID=>$this->getContractStatusId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerContractStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerContractStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerContractStatus[]
	 */
	public static function findByExample(PDO $db,VTigerContractStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerContractStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_contract_status`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerContractStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerContractStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerContractStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerContractStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_contract_status`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setContractStatusId($result['contract_statusid']);
	 ...
	 * @return VTigerContractStatus
	 */
	public static function findById(PDO $db,$contractStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$contractStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getContractStatusId());
		$stmt->bindValue(2,$this->getContractStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getContractStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getContractStatus());
		...
		if (false!==$lastInsertId) {
			$this->setContractStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getContractStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerContractStatus';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerContractStatus');
	}

	/**
	 * get single VTigerContractStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerContractStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerContractStatus') as $node) {
