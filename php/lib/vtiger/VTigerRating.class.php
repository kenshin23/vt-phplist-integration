 ...
 * @package entity
 */
class VTigerRating extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerRating';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_rating';
	const SQL_INSERT='INSERT INTO `vtiger_rating` (`rating_id`,`rating`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_rating` (`rating`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_rating` SET `rating_id`=?,`rating`=?,`presence`=?,`picklist_valueid`=? WHERE `rating_id`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_rating` WHERE `rating_id`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_rating` WHERE `rating_id`=?';
	const FIELD_RATING_ID=-68047006;
	const FIELD_RATING=1919476568;
	const FIELD_PRESENCE=372591606;
	const FIELD_PICKLIST_VALUEID=323861063;
	private static $PRIMARY_KEYS=array(self::FIELD_RATING_ID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_RATING_ID);
	private static $FIELD_NAMES=array(
		self::FIELD_RATING_ID=>'rating_id',
		self::FIELD_RATING=>'rating',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'picklist_valueid');
	private static $PROPERTY_NAMES=array(
		self::FIELD_RATING_ID=>'ratingId',
		self::FIELD_RATING=>'rating',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'pickListValueId');
	private static $PROPERTY_TYPES=array(
		self::FIELD_RATING_ID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_RATING_ID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
	...
	public function &setRatingId($ratingId) {
		$this->notifyChanged(self::FIELD_RATING_ID,$this->ratingId,$ratingId);
		$this->ratingId=$ratingId;
		return $this;
	}

	/**
	 * get value for rating_id 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_RATING_ID=>$this->getRatingId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerRating instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerRating $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerRating[]
	 */
	public static function findByExample(PDO $db,VTigerRating $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerRating instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerRating instances
	 *
	...
	/**
	 * returns the result as an array of VTigerRating instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerRating[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerRating instances.
	 *
	...
	public function assignByHash($result) {
		$this->setRatingId($result['rating_id']);
	 ...
	 * @return VTigerRating
	 */
	public static function findById(PDO $db,$ratingId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$ratingId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getRatingId());
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getRatingId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerRating');
	}

	/**
	 * get single VTigerRating instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerRating from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerRating') as $node) {
