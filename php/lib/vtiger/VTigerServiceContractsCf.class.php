 ...
 * @package entity
 */
class VTigerServiceContractsCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerServiceContractsCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_servicecontractscf';
	const SQL_INSERT='INSERT INTO `vtiger_servicecontractscf` (`servicecontractsid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_servicecontractscf` (`servicecontractsid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_servicecontractscf` SET `servicecontractsid`=? WHERE `servicecontractsid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_servicecontractscf` WHERE `servicecontractsid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_servicecontractscf` WHERE `servicecontractsid`=?';
	const FIELD_SERVICECONTRACTSID=439578448;
	...
	private static $FIELD_TYPES=array(
		self::FIELD_SERVICECONTRACTSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false));
	private static $DEFAULT_VALUES=array(
		self::FIELD_SERVICECONTRACTSID=>0);
	 ...
	 * @return VTigerServiceContractsCf
	 */
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerServiceContractsCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerServiceContractsCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerServiceContractsCf[]
	 */
	public static function findByExample(PDO $db,VTigerServiceContractsCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerServiceContractsCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_servicecontractscf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerServiceContractsCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerServiceContractsCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerServiceContractsCf[]
	 */
	public static function fromExecutedStatement(PDOStatement $stmt) {
		...
		while($result=$stmt->fetch(PDO::FETCH_ASSOC)) {
			$o=new VTigerServiceContractsCf();
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerServiceContractsCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_servicecontractscf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getServiceContractsId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerServiceContractsCf';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerServiceContractsCf');
	}

	/**
	 * get single VTigerServiceContractsCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerServiceContractsCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerServiceContractsCf') as $node) {
