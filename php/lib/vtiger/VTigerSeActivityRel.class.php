 ...
 * @package entity
 */
class VTigerSeActivityRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerSeActivityRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_seactivityrel';
	const SQL_INSERT='INSERT INTO `vtiger_seactivityrel` (`crmid`,`activityid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_seactivityrel` (`crmid`,`activityid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_seactivityrel` SET `crmid`=?,`activityid`=? WHERE `crmid`=? AND `activityid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_seactivityrel` WHERE `crmid`=? AND `activityid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_seactivityrel` WHERE `crmid`=? AND `activityid`=?';
	const FIELD_CRMID=1628030061;
	const FIELD_ACTIVITYID=1874410486;
	private static $PRIMARY_KEYS=array(self::FIELD_CRMID,self::FIELD_ACTIVITYID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerSeActivityRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerSeActivityRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerSeActivityRel[]
	 */
	public static function findByExample(PDO $db,VTigerSeActivityRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerSeActivityRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_seactivityrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerSeActivityRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerSeActivityRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerSeActivityRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerSeActivityRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_seactivityrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return VTigerSeActivityRel
	 */
	public static function findById(PDO $db,$crMid,$activityId) {
	...
	/**
	 * Fetch VTigerCRmEntity which references this VTigerSeActivityRel. Will return null in case reference is invalid.
	 * `vtiger_seactivityrel`.`crmid` -> `vtiger_crmentity`.`crmid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerSeActivityRel');
	}

	/**
	 * get single VTigerSeActivityRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerSeActivityRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerSeActivityRel') as $node) {
