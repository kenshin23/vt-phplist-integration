 ...
 * @package entity
 */
class VTigerIndustry extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerIndustry';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_industry';
	const SQL_INSERT='INSERT INTO `vtiger_industry` (`industryid`,`industry`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_industry` (`industry`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_industry` SET `industryid`=?,`industry`=?,`presence`=?,`picklist_valueid`=? WHERE `industryid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_industry` WHERE `industryid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_industry` WHERE `industryid`=?';
	const FIELD_INDUSTRYID=-902828461;
	const FIELD_INDUSTRY=1487327032;
	const FIELD_PRESENCE=83503701;
	const FIELD_PICKLIST_VALUEID=1538902438;
	private static $PRIMARY_KEYS=array(self::FIELD_INDUSTRYID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_INDUSTRYID);
	private static $FIELD_NAMES=array(
		self::FIELD_INDUSTRYID=>'industryid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_INDUSTRYID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_INDUSTRYID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_INDUSTRY=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	/**
	 * set value for industryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $industryId
	 * @return VTigerIndustry
	 */
	public function &setIndustryId($industryId) {
		$this->notifyChanged(self::FIELD_INDUSTRYID,$this->industryId,$industryId);
		$this->industryId=$industryId;
		return $this;
	}

	/**
	 * get value for industryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_INDUSTRYID=>$this->getIndustryId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerIndustry instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerIndustry $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerIndustry[]
	 */
	public static function findByExample(PDO $db,VTigerIndustry $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerIndustry instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_industry`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerIndustry instances
	 *
	...
	/**
	 * returns the result as an array of VTigerIndustry instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerIndustry[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerIndustry instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_industry`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setIndustryId($result['industryid']);
	 ...
	 * @return VTigerIndustry
	 */
	public static function findById(PDO $db,$industryId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$industryId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getIndustryId());
		$stmt->bindValue(2,$this->getIndustry());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getIndustryId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getIndustry());
		...
		if (false!==$lastInsertId) {
			$this->setIndustryId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getIndustryId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerIndustry');
	}

	/**
	 * get single VTigerIndustry instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerIndustry from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerIndustry') as $node) {
