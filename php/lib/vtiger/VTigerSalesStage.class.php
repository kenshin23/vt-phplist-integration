 ...
 * @package entity
 */
class VTigerSalesStage extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerSalesStage';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_sales_stage';
	const SQL_INSERT='INSERT INTO `vtiger_sales_stage` (`sales_stage_id`,`sales_stage`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_sales_stage` (`sales_stage`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_sales_stage` SET `sales_stage_id`=?,`sales_stage`=?,`presence`=?,`picklist_valueid`=? WHERE `sales_stage_id`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_sales_stage` WHERE `sales_stage_id`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_sales_stage` WHERE `sales_stage_id`=?';
	const FIELD_SALES_STAGE_ID=824120840;
	const FIELD_SALES_STAGE=-1850249614;
	const FIELD_PRESENCE=913298932;
	const FIELD_PICKLIST_VALUEID=349529669;
	private static $PRIMARY_KEYS=array(self::FIELD_SALES_STAGE_ID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_SALES_STAGE_ID);
	private static $FIELD_NAMES=array(
		self::FIELD_SALES_STAGE_ID=>'sales_stage_id',
		self::FIELD_SALES_STAGE=>'sales_stage',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'picklist_valueid');
	private static $PROPERTY_NAMES=array(
		self::FIELD_SALES_STAGE_ID=>'salesStageId',
		self::FIELD_SALES_STAGE=>'salesStage',
		self::FIELD_PRESENCE=>'presence',
		self::FIELD_PICKLIST_VALUEID=>'pickListValueId');
	private static $PROPERTY_TYPES=array(
		self::FIELD_SALES_STAGE_ID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_SALES_STAGE_ID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_SALES_STAGE_ID=>null,
		self::FIELD_SALES_STAGE=>null,
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $salesStageId;
	private $salesStage;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for sales_stage_id 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $salesStageId
	 * @return VTigerSalesStage
	 */
	public function &setSalesStageId($salesStageId) {
		$this->notifyChanged(self::FIELD_SALES_STAGE_ID,$this->salesStageId,$salesStageId);
		$this->salesStageId=$salesStageId;
		return $this;
	}

	/**
	 * get value for sales_stage_id 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_SALES_STAGE_ID=>$this->getSalesStageId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerSalesStage instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerSalesStage $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerSalesStage[]
	 */
	public static function findByExample(PDO $db,VTigerSalesStage $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerSalesStage instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_sales_stage`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerSalesStage instances
	 *
	...
	/**
	 * returns the result as an array of VTigerSalesStage instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerSalesStage[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerSalesStage instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_sales_stage`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setSalesStageId($result['sales_stage_id']);
	 ...
	 * @return VTigerSalesStage
	 */
	public static function findById(PDO $db,$salesStageId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$salesStageId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getSalesStageId());
		$stmt->bindValue(2,$this->getSalesStage());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getSalesStageId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getSalesStage());
		...
		if (false!==$lastInsertId) {
			$this->setSalesStageId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getSalesStageId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerSalesStage');
	}

	/**
	 * get single VTigerSalesStage instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerSalesStage from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerSalesStage') as $node) {
