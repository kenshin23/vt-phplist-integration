 ...
 * @package entity
 */
class VTigerProjectTaskPriority extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProjectTaskPriority';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_projecttaskpriority';
	const SQL_INSERT='INSERT INTO `vtiger_projecttaskpriority` (`projecttaskpriorityid`,`projecttaskpriority`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_projecttaskpriority` (`projecttaskpriority`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_projecttaskpriority` SET `projecttaskpriorityid`=?,`projecttaskpriority`=?,`presence`=?,`picklist_valueid`=? WHERE `projecttaskpriorityid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_projecttaskpriority` WHERE `projecttaskpriorityid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_projecttaskpriority` WHERE `projecttaskpriorityid`=?';
	const FIELD_PROJECTTASKPRIORITYID=-1305475973;
	const FIELD_PROJECTTASKPRIORITY=981880672;
	const FIELD_PRESENCE=336899101;
	const FIELD_PICKLIST_VALUEID=37356398;
	private static $PRIMARY_KEYS=array(self::FIELD_PROJECTTASKPRIORITYID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_PROJECTTASKPRIORITYID);
	private static $FIELD_NAMES=array(
		self::FIELD_PROJECTTASKPRIORITYID=>'projecttaskpriorityid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_PROJECTTASKPRIORITYID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PROJECTTASKPRIORITYID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_PROJECTTASKPRIORITY=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_PROJECTTASKPRIORITYID=>null,
		self::FIELD_PROJECTTASKPRIORITY=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $projectTaskPriorityId;
	private $projectTaskPriority;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for projecttaskpriorityid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $projectTaskPriorityId
	 * @return VTigerProjectTaskPriority
	 */
	public function &setProjectTaskPriorityId($projectTaskPriorityId) {
		$this->notifyChanged(self::FIELD_PROJECTTASKPRIORITYID,$this->projectTaskPriorityId,$projectTaskPriorityId);
		$this->projectTaskPriorityId=$projectTaskPriorityId;
		return $this;
	}

	/**
	 * get value for projecttaskpriorityid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getProjectTaskPriorityId() {
		return $this->projectTaskPriorityId;
	}
	...
	public function toArray() {
		return array(
			self::FIELD_PROJECTTASKPRIORITYID=>$this->getProjectTaskPriorityId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProjectTaskPriority instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProjectTaskPriority $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProjectTaskPriority[]
	 */
	public static function findByExample(PDO $db,VTigerProjectTaskPriority $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProjectTaskPriority instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_projecttaskpriority`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProjectTaskPriority instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProjectTaskPriority instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProjectTaskPriority[]
	 */
	public static function fromExecutedStatement(PDOStatement $stmt) {
		...
		while($result=$stmt->fetch(PDO::FETCH_ASSOC)) {
			$o=new VTigerProjectTaskPriority();
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProjectTaskPriority instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_projecttaskpriority`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setProjectTaskPriorityId($result['projecttaskpriorityid']);
	 ...
	 * @return VTigerProjectTaskPriority
	 */
	public static function findById(PDO $db,$projectTaskPriorityId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$projectTaskPriorityId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getProjectTaskPriorityId());
		$stmt->bindValue(2,$this->getProjectTaskPriority());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getProjectTaskPriorityId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getProjectTaskPriority());
		...
		if (false!==$lastInsertId) {
			$this->setProjectTaskPriorityId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getProjectTaskPriorityId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerProjectTaskPriority';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProjectTaskPriority');
	}

	/**
	 * get single VTigerProjectTaskPriority instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProjectTaskPriority from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProjectTaskPriority') as $node) {
