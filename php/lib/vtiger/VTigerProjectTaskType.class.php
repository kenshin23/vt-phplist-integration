 ...
 * @package entity
 */
class VTigerProjectTaskType extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProjectTaskType';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_projecttasktype';
	const SQL_INSERT='INSERT INTO `vtiger_projecttasktype` (`projecttasktypeid`,`projecttasktype`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_projecttasktype` (`projecttasktype`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_projecttasktype` SET `projecttasktypeid`=?,`projecttasktype`=?,`presence`=?,`picklist_valueid`=? WHERE `projecttasktypeid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_projecttasktype` WHERE `projecttasktypeid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_projecttasktype` WHERE `projecttasktypeid`=?';
	const FIELD_PROJECTTASKTYPEID=-347432281;
	const FIELD_PROJECTTASKTYPE=594051212;
	const FIELD_PRESENCE=1998146375;
	const FIELD_PICKLIST_VALUEID=-177612648;
	private static $PRIMARY_KEYS=array(self::FIELD_PROJECTTASKTYPEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_PROJECTTASKTYPEID);
	private static $FIELD_NAMES=array(
		self::FIELD_PROJECTTASKTYPEID=>'projecttasktypeid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_PROJECTTASKTYPEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PROJECTTASKTYPEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_PROJECTTASKTYPE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_PROJECTTASKTYPEID=>null,
		self::FIELD_PROJECTTASKTYPE=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $projectTaskTypeId;
	private $projectTaskType;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for projecttasktypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $projectTaskTypeId
	 * @return VTigerProjectTaskType
	 */
	public function &setProjectTaskTypeId($projectTaskTypeId) {
		$this->notifyChanged(self::FIELD_PROJECTTASKTYPEID,$this->projectTaskTypeId,$projectTaskTypeId);
		$this->projectTaskTypeId=$projectTaskTypeId;
		return $this;
	}

	/**
	 * get value for projecttasktypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getProjectTaskTypeId() {
	...
	public function toArray() {
		return array(
			self::FIELD_PROJECTTASKTYPEID=>$this->getProjectTaskTypeId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProjectTaskType instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProjectTaskType $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProjectTaskType[]
	 */
	public static function findByExample(PDO $db,VTigerProjectTaskType $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProjectTaskType instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_projecttasktype`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProjectTaskType instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProjectTaskType instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProjectTaskType[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProjectTaskType instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_projecttasktype`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setProjectTaskTypeId($result['projecttasktypeid']);
	 ...
	 * @return VTigerProjectTaskType
	 */
	public static function findById(PDO $db,$projectTaskTypeId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$projectTaskTypeId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getProjectTaskTypeId());
		$stmt->bindValue(2,$this->getProjectTaskType());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getProjectTaskTypeId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getProjectTaskType());
		...
		if (false!==$lastInsertId) {
			$this->setProjectTaskTypeId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getProjectTaskTypeId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerProjectTaskType';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProjectTaskType');
	}

	/**
	 * get single VTigerProjectTaskType instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProjectTaskType from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProjectTaskType') as $node) {
