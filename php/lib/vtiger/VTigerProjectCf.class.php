 ...
 * @package entity
 */
class VTigerProjectCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProjectCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_projectcf';
	const SQL_INSERT='INSERT INTO `vtiger_projectcf` (`projectid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_projectcf` (`projectid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_projectcf` SET `projectid`=? WHERE `projectid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_projectcf` WHERE `projectid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_projectcf` WHERE `projectid`=?';
	const FIELD_PROJECTID=1518853260;
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PROJECTID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false));
	private static $DEFAULT_VALUES=array(
		self::FIELD_PROJECTID=>0);
	private $projectId;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProjectCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProjectCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProjectCf[]
	 */
	public static function findByExample(PDO $db,VTigerProjectCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProjectCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_projectcf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProjectCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProjectCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProjectCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProjectCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_projectcf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getProjectId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProjectCf');
	}

	/**
	 * get single VTigerProjectCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProjectCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProjectCf') as $node) {
