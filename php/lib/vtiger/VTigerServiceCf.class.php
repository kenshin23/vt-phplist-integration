 ...
 * @package entity
 */
class VTigerServiceCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerServiceCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_servicecf';
	const SQL_INSERT='INSERT INTO `vtiger_servicecf` (`serviceid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_servicecf` (`serviceid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_servicecf` SET `serviceid`=? WHERE `serviceid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_servicecf` WHERE `serviceid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_servicecf` WHERE `serviceid`=?';
	const FIELD_SERVICEID=-462040380;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerServiceCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerServiceCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerServiceCf[]
	 */
	public static function findByExample(PDO $db,VTigerServiceCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerServiceCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_servicecf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerServiceCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerServiceCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerServiceCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerServiceCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_servicecf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getServiceId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerServiceCf');
	}

	/**
	 * get single VTigerServiceCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerServiceCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerServiceCf') as $node) {
