 ...
 * @package entity
 */
class VTigerProjectTaskCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProjectTaskCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_projecttaskcf';
	const SQL_INSERT='INSERT INTO `vtiger_projecttaskcf` (`projecttaskid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_projecttaskcf` (`projecttaskid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_projecttaskcf` SET `projecttaskid`=? WHERE `projecttaskid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_projecttaskcf` WHERE `projecttaskid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_projecttaskcf` WHERE `projecttaskid`=?';
	const FIELD_PROJECTTASKID=-2138241450;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProjectTaskCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProjectTaskCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProjectTaskCf[]
	 */
	public static function findByExample(PDO $db,VTigerProjectTaskCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProjectTaskCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_projecttaskcf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProjectTaskCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProjectTaskCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProjectTaskCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProjectTaskCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_projecttaskcf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(2,$this->getProjectTaskId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProjectTaskCf');
	}

	/**
	 * get single VTigerProjectTaskCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProjectTaskCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProjectTaskCf') as $node) {
