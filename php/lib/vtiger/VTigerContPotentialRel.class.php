 ...
 * @package entity
 */
class VTigerContPotentialRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerContPotentialRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_contpotentialrel';
	const SQL_INSERT='INSERT INTO `vtiger_contpotentialrel` (`contactid`,`potentialid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_contpotentialrel` (`contactid`,`potentialid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_contpotentialrel` SET `contactid`=?,`potentialid`=? WHERE `contactid`=? AND `potentialid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_contpotentialrel` WHERE `contactid`=? AND `potentialid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_contpotentialrel` WHERE `contactid`=? AND `potentialid`=?';
	const FIELD_CONTACTID=1933523056;
	const FIELD_POTENTIALID=1601033134;
	private static $PRIMARY_KEYS=array(self::FIELD_CONTACTID,self::FIELD_POTENTIALID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerContPotentialRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerContPotentialRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerContPotentialRel[]
	 */
	public static function findByExample(PDO $db,VTigerContPotentialRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerContPotentialRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_contpotentialrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerContPotentialRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerContPotentialRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerContPotentialRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerContPotentialRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_contpotentialrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return VTigerContPotentialRel
	 */
	public static function findById(PDO $db,$contactId,$potentialId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$contactId);
		$stmt->bindValue(2,$potentialId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function updateToDatabase(PDO $db) {
		...
		$stmt->bindValue(4,$this->getPotentialId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerPotential which references this VTigerContPotentialRel. Will return null in case reference is invalid.
	 * `vtiger_contpotentialrel`.`potentialid` -> `vtiger_potential`.`potentialid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerContPotentialRel';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerContPotentialRel');
	}

	/**
	 * get single VTigerContPotentialRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerContPotentialRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerContPotentialRel') as $node) {
