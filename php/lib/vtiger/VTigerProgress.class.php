 ...
 * @package entity
 */
class VTigerProgress extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProgress';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_progress';
	const SQL_INSERT='INSERT INTO `vtiger_progress` (`progressid`,`progress`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_progress` (`progress`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_progress` SET `progressid`=?,`progress`=?,`presence`=?,`picklist_valueid`=? WHERE `progressid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_progress` WHERE `progressid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_progress` WHERE `progressid`=?';
	const FIELD_PROGRESSID=1452482355;
	const FIELD_PROGRESS=-874465256;
	const FIELD_PRESENCE=-1150053658;
	const FIELD_PICKLIST_VALUEID=1359876407;
	private static $PRIMARY_KEYS=array(self::FIELD_PROGRESSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_PROGRESSID);
	private static $FIELD_NAMES=array(
		self::FIELD_PROGRESSID=>'progressid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_PROGRESSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PROGRESSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_PROGRESS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	/**
	 * set value for progressid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $progressId
	 * @return VTigerProgress
	 */
	public function &setProgressId($progressId) {
		$this->notifyChanged(self::FIELD_PROGRESSID,$this->progressId,$progressId);
		$this->progressId=$progressId;
		return $this;
	}

	/**
	 * get value for progressid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_PROGRESSID=>$this->getProgressId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProgress instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProgress $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProgress[]
	 */
	public static function findByExample(PDO $db,VTigerProgress $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProgress instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_progress`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProgress instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProgress instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProgress[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProgress instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_progress`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setProgressId($result['progressid']);
	 ...
	 * @return VTigerProgress
	 */
	public static function findById(PDO $db,$progressId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$progressId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getProgressId());
		$stmt->bindValue(2,$this->getProgress());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getProgressId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getProgress());
		...
		if (false!==$lastInsertId) {
			$this->setProgressId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getProgressId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProgress');
	}

	/**
	 * get single VTigerProgress instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProgress from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProgress') as $node) {
