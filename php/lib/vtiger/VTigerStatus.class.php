 ...
 * @package entity
 */
class VTigerStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_status';
	const SQL_INSERT='INSERT INTO `vtiger_status` (`statusid`,`status`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_status` (`status`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_status` SET `statusid`=?,`status`=?,`presence`=?,`picklist_valueid`=? WHERE `statusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_status` WHERE `statusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_status` WHERE `statusid`=?';
	const FIELD_STATUSID=-187463469;
	const FIELD_STATUS=1416563128;
	const FIELD_PRESENCE=1512144193;
	const FIELD_PICKLIST_VALUEID=881160338;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerStatus[]
	 */
	public static function findByExample(PDO $db,VTigerStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerStatus instances.
	 *
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerStatus');
	}

	/**
	 * get single VTigerStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerStatus') as $node) {
