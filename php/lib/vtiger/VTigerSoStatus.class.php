 ...
 * @package entity
 */
class VTigerSoStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerSoStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_sostatus';
	const SQL_INSERT='INSERT INTO `vtiger_sostatus` (`sostatusid`,`sostatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_sostatus` (`sostatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_sostatus` SET `sostatusid`=?,`sostatus`=?,`presence`=?,`picklist_valueid`=? WHERE `sostatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_sostatus` WHERE `sostatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_sostatus` WHERE `sostatusid`=?';
	const FIELD_SOSTATUSID=-1653224365;
	const FIELD_SOSTATUS=1607216440;
	const FIELD_PRESENCE=-1360322331;
	const FIELD_PICKLIST_VALUEID=1054027830;
	private static $PRIMARY_KEYS=array(self::FIELD_SOSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_SOSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_SOSTATUSID=>'sostatusid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_SOSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_SOSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_SOSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	/**
	 * set value for sostatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $soStatusId
	 * @return VTigerSoStatus
	 */
	public function &setSoStatusId($soStatusId) {
		$this->notifyChanged(self::FIELD_SOSTATUSID,$this->soStatusId,$soStatusId);
		$this->soStatusId=$soStatusId;
		return $this;
	}

	/**
	 * get value for sostatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getSoStatusId() {
	...
	public function toArray() {
		return array(
			self::FIELD_SOSTATUSID=>$this->getSoStatusId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerSoStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerSoStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerSoStatus[]
	 */
	public static function findByExample(PDO $db,VTigerSoStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerSoStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_sostatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerSoStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerSoStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerSoStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerSoStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_sostatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setSoStatusId($result['sostatusid']);
	 ...
	 * @return VTigerSoStatus
	 */
	public static function findById(PDO $db,$soStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$soStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getSoStatusId());
		$stmt->bindValue(2,$this->getSoStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getSoStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getSoStatus());
		...
		if (false!==$lastInsertId) {
			$this->setSoStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getSoStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerSoStatus');
	}

	/**
	 * get single VTigerSoStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerSoStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerSoStatus') as $node) {
