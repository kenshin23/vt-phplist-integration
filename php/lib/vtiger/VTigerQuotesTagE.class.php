 ...
 * @package entity
 */
class VTigerQuotesTagE extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerQuotesTagE';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_quotestage';
	const SQL_INSERT='INSERT INTO `vtiger_quotestage` (`quotestageid`,`quotestage`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_quotestage` (`quotestage`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_quotestage` SET `quotestageid`=?,`quotestage`=?,`presence`=?,`picklist_valueid`=? WHERE `quotestageid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_quotestage` WHERE `quotestageid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_quotestage` WHERE `quotestageid`=?';
	const FIELD_QUOTESTAGEID=1316755155;
	const FIELD_QUOTESTAGE=2061703096;
	const FIELD_PRESENCE=-230066127;
	const FIELD_PICKLIST_VALUEID=-1975813246;
	private static $PRIMARY_KEYS=array(self::FIELD_QUOTESTAGEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_QUOTESTAGEID);
	private static $FIELD_NAMES=array(
		self::FIELD_QUOTESTAGEID=>'quotestageid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_QUOTESTAGEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_QUOTESTAGEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_QUOTESTAGE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_QUOTESTAGEID=>null,
		self::FIELD_QUOTESTAGE=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $quotesTagEId;
	private $quotesTagE;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for quotestageid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $quotesTagEId
	 * @return VTigerQuotesTagE
	 */
	public function &setQuotesTagEId($quotesTagEId) {
		$this->notifyChanged(self::FIELD_QUOTESTAGEID,$this->quotesTagEId,$quotesTagEId);
		$this->quotesTagEId=$quotesTagEId;
		return $this;
	}

	/**
	 * get value for quotestageid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getQuotesTagEId() {
		return $this->quotesTagEId;
	}
	...
	public function toArray() {
		return array(
			self::FIELD_QUOTESTAGEID=>$this->getQuotesTagEId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerQuotesTagE instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerQuotesTagE $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerQuotesTagE[]
	 */
	public static function findByExample(PDO $db,VTigerQuotesTagE $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerQuotesTagE instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerQuotesTagE instances
	 *
	...
	/**
	 * returns the result as an array of VTigerQuotesTagE instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerQuotesTagE[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerQuotesTagE instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_quotestage`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setQuotesTagEId($result['quotestageid']);
	 ...
	 * @return VTigerQuotesTagE
	 */
	public static function findById(PDO $db,$quotesTagEId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$quotesTagEId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getQuotesTagEId());
		$stmt->bindValue(2,$this->getQuotesTagE());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getQuotesTagEId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getQuotesTagE());
		...
		if (false!==$lastInsertId) {
			$this->setQuotesTagEId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getQuotesTagEId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerQuotesTagE');
	}

	/**
	 * get single VTigerQuotesTagE instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerQuotesTagE from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerQuotesTagE') as $node) {
