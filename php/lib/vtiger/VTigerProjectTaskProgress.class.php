 ...
 * @package entity
 */
class VTigerProjectTaskProgress extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProjectTaskProgress';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_projecttaskprogress';
	const SQL_INSERT='INSERT INTO `vtiger_projecttaskprogress` (`projecttaskprogressid`,`projecttaskprogress`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_projecttaskprogress` (`projecttaskprogress`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_projecttaskprogress` SET `projecttaskprogressid`=?,`projecttaskprogress`=?,`presence`=?,`picklist_valueid`=? WHERE `projecttaskprogressid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_projecttaskprogress` WHERE `projecttaskprogressid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_projecttaskprogress` WHERE `projecttaskprogressid`=?';
	const FIELD_PROJECTTASKPROGRESSID=-1971479923;
	const FIELD_PROJECTTASKPROGRESS=856048114;
	const FIELD_PRESENCE=-1293488716;
	const FIELD_PICKLIST_VALUEID=-2087141883;
	private static $PRIMARY_KEYS=array(self::FIELD_PROJECTTASKPROGRESSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_PROJECTTASKPROGRESSID);
	private static $FIELD_NAMES=array(
		self::FIELD_PROJECTTASKPROGRESSID=>'projecttaskprogressid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_PROJECTTASKPROGRESSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PROJECTTASKPROGRESSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_PROJECTTASKPROGRESS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_PROJECTTASKPROGRESSID=>null,
		self::FIELD_PROJECTTASKPROGRESS=>'',
		self::FIELD_PRESENCE=>1,
		self::FIELD_PICKLIST_VALUEID=>0);
	private $projectTaskProgressId;
	private $projectTaskProgress;
	private $presence;
	private $pickListValueId;

	/**
	 * set value for projecttaskprogressid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $projectTaskProgressId
	 * @return VTigerProjectTaskProgress
	 */
	public function &setProjectTaskProgressId($projectTaskProgressId) {
		$this->notifyChanged(self::FIELD_PROJECTTASKPROGRESSID,$this->projectTaskProgressId,$projectTaskProgressId);
		$this->projectTaskProgressId=$projectTaskProgressId;
		return $this;
	}

	/**
	 * get value for projecttaskprogressid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getProjectTaskProgressId() {
		return $this->projectTaskProgressId;
	}
	...
	public function toArray() {
		return array(
			self::FIELD_PROJECTTASKPROGRESSID=>$this->getProjectTaskProgressId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProjectTaskProgress instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProjectTaskProgress $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProjectTaskProgress[]
	 */
	public static function findByExample(PDO $db,VTigerProjectTaskProgress $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProjectTaskProgress instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_projecttaskprogress`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProjectTaskProgress instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProjectTaskProgress instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProjectTaskProgress[]
	 */
	public static function fromExecutedStatement(PDOStatement $stmt) {
		...
		while($result=$stmt->fetch(PDO::FETCH_ASSOC)) {
			$o=new VTigerProjectTaskProgress();
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProjectTaskProgress instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_projecttaskprogress`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setProjectTaskProgressId($result['projecttaskprogressid']);
	 ...
	 * @return VTigerProjectTaskProgress
	 */
	public static function findById(PDO $db,$projectTaskProgressId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$projectTaskProgressId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getProjectTaskProgressId());
		$stmt->bindValue(2,$this->getProjectTaskProgress());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getProjectTaskProgressId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getProjectTaskProgress());
		...
		if (false!==$lastInsertId) {
			$this->setProjectTaskProgressId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getProjectTaskProgressId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerProjectTaskProgress';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProjectTaskProgress');
	}

	/**
	 * get single VTigerProjectTaskProgress instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProjectTaskProgress from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProjectTaskProgress') as $node) {
