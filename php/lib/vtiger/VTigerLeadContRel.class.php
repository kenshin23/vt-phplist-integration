 ...
 * @package entity
 */
class VTigerLeadContRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerLeadContRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_leadcontrel';
	const SQL_INSERT='INSERT INTO `vtiger_leadcontrel` (`leadid`,`contactid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_leadcontrel` (`leadid`,`contactid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_leadcontrel` SET `leadid`=?,`contactid`=? WHERE `leadid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_leadcontrel` WHERE `leadid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_leadcontrel` WHERE `leadid`=?';
	const FIELD_LEADID=1408381296;
	const FIELD_CONTACTID=-2009461086;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerLeadContRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerLeadContRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerLeadContRel[]
	 */
	public static function findByExample(PDO $db,VTigerLeadContRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerLeadContRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_leadcontrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerLeadContRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerLeadContRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerLeadContRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerLeadContRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_leadcontrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(3,$this->getLeadId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerLeadDetails which references this VTigerLeadContRel. Will return null in case reference is invalid.
	 * `vtiger_leadcontrel`.`leadid` -> `vtiger_leaddetails`.`leadid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerLeadContRel');
	}

	/**
	 * get single VTigerLeadContRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerLeadContRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerLeadContRel') as $node) {
