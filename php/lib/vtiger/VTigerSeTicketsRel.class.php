 ...
 * @package entity
 */
class VTigerSeTicketsRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerSeTicketsRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_seticketsrel';
	const SQL_INSERT='INSERT INTO `vtiger_seticketsrel` (`crmid`,`ticketid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_seticketsrel` (`crmid`,`ticketid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_seticketsrel` SET `crmid`=?,`ticketid`=? WHERE `crmid`=? AND `ticketid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_seticketsrel` WHERE `crmid`=? AND `ticketid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_seticketsrel` WHERE `crmid`=? AND `ticketid`=?';
	const FIELD_CRMID=1044212517;
	const FIELD_TICKETID=-431924741;
	private static $PRIMARY_KEYS=array(self::FIELD_CRMID,self::FIELD_TICKETID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerSeTicketsRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerSeTicketsRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerSeTicketsRel[]
	 */
	public static function findByExample(PDO $db,VTigerSeTicketsRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerSeTicketsRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_seticketsrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerSeTicketsRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerSeTicketsRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerSeTicketsRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerSeTicketsRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_seticketsrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return VTigerSeTicketsRel
	 */
	public static function findById(PDO $db,$crMid,$ticketId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$crMid);
		$stmt->bindValue(2,$ticketId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function updateToDatabase(PDO $db) {
		...
		$stmt->bindValue(4,$this->getTicketId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerTroubleTickets which references this VTigerSeTicketsRel. Will return null in case reference is invalid.
	 * `vtiger_seticketsrel`.`ticketid` -> `vtiger_troubletickets`.`ticketid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerSeTicketsRel');
	}

	/**
	 * get single VTigerSeTicketsRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerSeTicketsRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerSeTicketsRel') as $node) {
