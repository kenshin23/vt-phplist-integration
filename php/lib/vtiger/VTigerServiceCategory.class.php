 ...
 * @package entity
 */
class VTigerServiceCategory extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerServiceCategory';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_servicecategory';
	const SQL_INSERT='INSERT INTO `vtiger_servicecategory` (`servicecategoryid`,`servicecategory`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_servicecategory` (`servicecategory`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_servicecategory` SET `servicecategoryid`=?,`servicecategory`=?,`presence`=?,`picklist_valueid`=? WHERE `servicecategoryid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_servicecategory` WHERE `servicecategoryid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_servicecategory` WHERE `servicecategoryid`=?';
	const FIELD_SERVICECATEGORYID=-1258523683;
	const FIELD_SERVICECATEGORY=1259024194;
	const FIELD_PRESENCE=1956667212;
	const FIELD_PICKLIST_VALUEID=-1175251043;
	private static $PRIMARY_KEYS=array(self::FIELD_SERVICECATEGORYID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_SERVICECATEGORYID);
	private static $FIELD_NAMES=array(
		self::FIELD_SERVICECATEGORYID=>'servicecategoryid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_SERVICECATEGORYID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_SERVICECATEGORYID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_SERVICECATEGORY=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_SERVICECATEGORYID=>null,
		self::FIELD_SERVICECATEGORY=>'',
	...
	/**
	 * set value for servicecategoryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $serviceCategoryId
	 * @return VTigerServiceCategory
	 */
	public function &setServiceCategoryId($serviceCategoryId) {
		$this->notifyChanged(self::FIELD_SERVICECATEGORYID,$this->serviceCategoryId,$serviceCategoryId);
		$this->serviceCategoryId=$serviceCategoryId;
		return $this;
	}

	/**
	 * get value for servicecategoryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_SERVICECATEGORYID=>$this->getServiceCategoryId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerServiceCategory instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerServiceCategory $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerServiceCategory[]
	 */
	public static function findByExample(PDO $db,VTigerServiceCategory $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerServiceCategory instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_servicecategory`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerServiceCategory instances
	 *
	...
	/**
	 * returns the result as an array of VTigerServiceCategory instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerServiceCategory[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerServiceCategory instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_servicecategory`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setServiceCategoryId($result['servicecategoryid']);
	 ...
	 * @return VTigerServiceCategory
	 */
	public static function findById(PDO $db,$serviceCategoryId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$serviceCategoryId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getServiceCategoryId());
		$stmt->bindValue(2,$this->getServiceCategory());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getServiceCategoryId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getServiceCategory());
		...
		if (false!==$lastInsertId) {
			$this->setServiceCategoryId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getServiceCategoryId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerServiceCategory';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerServiceCategory');
	}

	/**
	 * get single VTigerServiceCategory instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerServiceCategory from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerServiceCategory') as $node) {
