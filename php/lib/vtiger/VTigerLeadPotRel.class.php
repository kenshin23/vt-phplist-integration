 ...
 * @package entity
 */
class VTigerLeadPotRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerLeadPotRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_leadpotrel';
	const SQL_INSERT='INSERT INTO `vtiger_leadpotrel` (`leadid`,`potentialid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_leadpotrel` (`leadid`,`potentialid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_leadpotrel` SET `leadid`=?,`potentialid`=? WHERE `potentialid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_leadpotrel` WHERE `potentialid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_leadpotrel` WHERE `potentialid`=?';
	const FIELD_LEADID=-1098749457;
	const FIELD_POTENTIALID=949363777;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerLeadPotRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerLeadPotRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerLeadPotRel[]
	 */
	public static function findByExample(PDO $db,VTigerLeadPotRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerLeadPotRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_leadpotrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerLeadPotRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerLeadPotRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerLeadPotRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerLeadPotRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_leadpotrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	/**
	 * Fetch VTigerPotential which references this VTigerLeadPotRel. Will return null in case reference is invalid.
	 * `vtiger_leadpotrel`.`potentialid` -> `vtiger_potential`.`potentialid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerLeadPotRel');
	}

	/**
	 * get single VTigerLeadPotRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerLeadPotRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerLeadPotRel') as $node) {
