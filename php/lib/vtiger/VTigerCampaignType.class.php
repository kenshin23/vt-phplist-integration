 ...
 * @package entity
 */
class VTigerCampaignType extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerCampaignType';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_campaigntype';
	const SQL_INSERT='INSERT INTO `vtiger_campaigntype` (`campaigntypeid`,`campaigntype`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_campaigntype` (`campaigntype`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_campaigntype` SET `campaigntypeid`=?,`campaigntype`=?,`presence`=?,`picklist_valueid`=? WHERE `campaigntypeid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_campaigntype` WHERE `campaigntypeid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_campaigntype` WHERE `campaigntypeid`=?';
	const FIELD_CAMPAIGNTYPEID=663940051;
	const FIELD_CAMPAIGNTYPE=-441766728;
	const FIELD_PRESENCE=1596565609;
	const FIELD_PICKLIST_VALUEID=1524888506;
	private static $PRIMARY_KEYS=array(self::FIELD_CAMPAIGNTYPEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_CAMPAIGNTYPEID);
	private static $FIELD_NAMES=array(
		self::FIELD_CAMPAIGNTYPEID=>'campaigntypeid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_CAMPAIGNTYPEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_CAMPAIGNTYPEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_CAMPAIGNTYPE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_CAMPAIGNTYPEID=>null,
		self::FIELD_CAMPAIGNTYPE=>'',
	...
	/**
	 * set value for campaigntypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $campaignTypeId
	 * @return VTigerCampaignType
	 */
	public function &setCampaignTypeId($campaignTypeId) {
		$this->notifyChanged(self::FIELD_CAMPAIGNTYPEID,$this->campaignTypeId,$campaignTypeId);
		$this->campaignTypeId=$campaignTypeId;
		return $this;
	}

	/**
	 * get value for campaigntypeid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_CAMPAIGNTYPEID=>$this->getCampaignTypeId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerCampaignType instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerCampaignType $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerCampaignType[]
	 */
	public static function findByExample(PDO $db,VTigerCampaignType $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerCampaignType instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_campaigntype`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerCampaignType instances
	 *
	...
	/**
	 * returns the result as an array of VTigerCampaignType instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerCampaignType[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerCampaignType instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_campaigntype`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setCampaignTypeId($result['campaigntypeid']);
	 ...
	 * @return VTigerCampaignType
	 */
	public static function findById(PDO $db,$campaignTypeId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$campaignTypeId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getCampaignTypeId());
		$stmt->bindValue(2,$this->getCampaignType());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getCampaignTypeId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getCampaignType());
		...
		if (false!==$lastInsertId) {
			$this->setCampaignTypeId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getCampaignTypeId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerCampaignType');
	}

	/**
	 * get single VTigerCampaignType instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerCampaignType from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerCampaignType') as $node) {
