 ...
 * @package entity
 */
class VTigerProductCategory extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerProductCategory';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_productcategory';
	const SQL_INSERT='INSERT INTO `vtiger_productcategory` (`productcategoryid`,`productcategory`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_productcategory` (`productcategory`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_productcategory` SET `productcategoryid`=?,`productcategory`=?,`presence`=?,`picklist_valueid`=? WHERE `productcategoryid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_productcategory` WHERE `productcategoryid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_productcategory` WHERE `productcategoryid`=?';
	const FIELD_PRODUCTCATEGORYID=-1588343983;
	const FIELD_PRODUCTCATEGORY=-663104586;
	const FIELD_PRESENCE=787502162;
	const FIELD_PICKLIST_VALUEID=-1390893405;
	private static $PRIMARY_KEYS=array(self::FIELD_PRODUCTCATEGORYID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_PRODUCTCATEGORYID);
	private static $FIELD_NAMES=array(
		self::FIELD_PRODUCTCATEGORYID=>'productcategoryid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_PRODUCTCATEGORYID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_PRODUCTCATEGORYID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_PRODUCTCATEGORY=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_PRODUCTCATEGORYID=>null,
		self::FIELD_PRODUCTCATEGORY=>'',
	...
	/**
	 * set value for productcategoryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $productCategoryId
	 * @return VTigerProductCategory
	 */
	public function &setProductCategoryId($productCategoryId) {
		$this->notifyChanged(self::FIELD_PRODUCTCATEGORYID,$this->productCategoryId,$productCategoryId);
		$this->productCategoryId=$productCategoryId;
		return $this;
	}

	/**
	 * get value for productcategoryid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_PRODUCTCATEGORYID=>$this->getProductCategoryId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerProductCategory instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerProductCategory $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerProductCategory[]
	 */
	public static function findByExample(PDO $db,VTigerProductCategory $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerProductCategory instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerProductCategory instances
	 *
	...
	/**
	 * returns the result as an array of VTigerProductCategory instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerProductCategory[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerProductCategory instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_productcategory`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setProductCategoryId($result['productcategoryid']);
	 ...
	 * @return VTigerProductCategory
	 */
	public static function findById(PDO $db,$productCategoryId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$productCategoryId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getProductCategoryId());
		$stmt->bindValue(2,$this->getProductCategory());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getProductCategoryId()) {
			...
			$stmt->bindValue(1,$this->getProductCategory());
		...
		if (false!==$lastInsertId) {
			$this->setProductCategoryId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getProductCategoryId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerProductCategory';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerProductCategory');
	}

	/**
	 * get single VTigerProductCategory instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerProductCategory from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerProductCategory') as $node) {
