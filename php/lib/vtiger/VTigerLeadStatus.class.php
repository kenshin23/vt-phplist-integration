 ...
 * @package entity
 */
class VTigerLeadStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerLeadStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_leadstatus';
	const SQL_INSERT='INSERT INTO `vtiger_leadstatus` (`leadstatusid`,`leadstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_leadstatus` (`leadstatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_leadstatus` SET `leadstatusid`=?,`leadstatus`=?,`presence`=?,`picklist_valueid`=? WHERE `leadstatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_leadstatus` WHERE `leadstatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_leadstatus` WHERE `leadstatusid`=?';
	const FIELD_LEADSTATUSID=12062547;
	const FIELD_LEADSTATUS=616771640;
	const FIELD_PRESENCE=1296614757;
	const FIELD_PICKLIST_VALUEID=696538294;
	private static $PRIMARY_KEYS=array(self::FIELD_LEADSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_LEADSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_LEADSTATUSID=>'leadstatusid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_LEADSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_LEADSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_LEADSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_LEADSTATUSID=>null,
		self::FIELD_LEADSTATUS=>'',
	...
	/**
	 * set value for leadstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $leadStatusId
	 * @return VTigerLeadStatus
	 */
	public function &setLeadStatusId($leadStatusId) {
		$this->notifyChanged(self::FIELD_LEADSTATUSID,$this->leadStatusId,$leadStatusId);
		$this->leadStatusId=$leadStatusId;
		return $this;
	}

	/**
	 * get value for leadstatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_LEADSTATUSID=>$this->getLeadStatusId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerLeadStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerLeadStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerLeadStatus[]
	 */
	public static function findByExample(PDO $db,VTigerLeadStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerLeadStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_leadstatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerLeadStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerLeadStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerLeadStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerLeadStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_leadstatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setLeadStatusId($result['leadstatusid']);
	 ...
	 * @return VTigerLeadStatus
	 */
	public static function findById(PDO $db,$leadStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$leadStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getLeadStatusId());
		$stmt->bindValue(2,$this->getLeadStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getLeadStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getLeadStatus());
		...
		if (false!==$lastInsertId) {
			$this->setLeadStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getLeadStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerLeadStatus');
	}

	/**
	 * get single VTigerLeadStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerLeadStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerLeadStatus') as $node) {
