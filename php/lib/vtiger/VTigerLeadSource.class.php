 ...
 * @package entity
 */
class VTigerLeadSource extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerLeadSource';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_leadsource';
	const SQL_INSERT='INSERT INTO `vtiger_leadsource` (`leadsourceid`,`leadsource`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_leadsource` (`leadsource`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_leadsource` SET `leadsourceid`=?,`leadsource`=?,`presence`=?,`picklist_valueid`=? WHERE `leadsourceid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_leadsource` WHERE `leadsourceid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_leadsource` WHERE `leadsourceid`=?';
	const FIELD_LEADSOURCEID=569308083;
	const FIELD_LEADSOURCE=-853037928;
	const FIELD_PRESENCE=-1565242532;
	const FIELD_PICKLIST_VALUEID=2141320109;
	private static $PRIMARY_KEYS=array(self::FIELD_LEADSOURCEID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_LEADSOURCEID);
	private static $FIELD_NAMES=array(
		self::FIELD_LEADSOURCEID=>'leadsourceid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_LEADSOURCEID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_LEADSOURCEID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_LEADSOURCE=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_LEADSOURCEID=>null,
		self::FIELD_LEADSOURCE=>'',
	...
	/**
	 * set value for leadsourceid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $leadSourceId
	 * @return VTigerLeadSource
	 */
	public function &setLeadSourceId($leadSourceId) {
		$this->notifyChanged(self::FIELD_LEADSOURCEID,$this->leadSourceId,$leadSourceId);
		$this->leadSourceId=$leadSourceId;
		return $this;
	}

	/**
	 * get value for leadsourceid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	...
	public function toArray() {
		return array(
			self::FIELD_LEADSOURCEID=>$this->getLeadSourceId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerLeadSource instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerLeadSource $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerLeadSource[]
	 */
	public static function findByExample(PDO $db,VTigerLeadSource $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerLeadSource instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_leadsource`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerLeadSource instances
	 *
	...
	/**
	 * returns the result as an array of VTigerLeadSource instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerLeadSource[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerLeadSource instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_leadsource`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setLeadSourceId($result['leadsourceid']);
	 ...
	 * @return VTigerLeadSource
	 */
	public static function findById(PDO $db,$leadSourceId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$leadSourceId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getLeadSourceId());
		$stmt->bindValue(2,$this->getLeadSource());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getLeadSourceId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getLeadSource());
		...
		if (false!==$lastInsertId) {
			$this->setLeadSourceId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getLeadSourceId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerLeadSource');
	}

	/**
	 * get single VTigerLeadSource instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerLeadSource from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerLeadSource') as $node) {
