 ...
 * @package entity
 */
class VTigerTicketCf extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerTicketCf';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_ticketcf';
	const SQL_INSERT='INSERT INTO `vtiger_ticketcf` (`ticketid`) VALUES (?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_ticketcf` (`ticketid`) VALUES (?)';
	const SQL_UPDATE='UPDATE `vtiger_ticketcf` SET `ticketid`=? WHERE `ticketid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_ticketcf` WHERE `ticketid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_ticketcf` WHERE `ticketid`=?';
	const FIELD_TICKETID=-1049222384;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerTicketCf instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerTicketCf $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerTicketCf[]
	 */
	public static function findByExample(PDO $db,VTigerTicketCf $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerTicketCf instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_ticketcf`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerTicketCf instances
	 *
	...
	/**
	 * returns the result as an array of VTigerTicketCf instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerTicketCf[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerTicketCf instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_ticketcf`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	/**
	 * Fetch VTigerTroubleTickets which references this VTigerTicketCf. Will return null in case reference is invalid.
	 * `vtiger_ticketcf`.`ticketid` -> `vtiger_troubletickets`.`ticketid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerTicketCf');
	}

	/**
	 * get single VTigerTicketCf instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerTicketCf from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerTicketCf') as $node) {
