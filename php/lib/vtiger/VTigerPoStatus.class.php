 ...
 * @package entity
 */
class VTigerPoStatus extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerPoStatus';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_postatus';
	const SQL_INSERT='INSERT INTO `vtiger_postatus` (`postatusid`,`postatus`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_postatus` (`postatus`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_postatus` SET `postatusid`=?,`postatus`=?,`presence`=?,`picklist_valueid`=? WHERE `postatusid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_postatus` WHERE `postatusid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_postatus` WHERE `postatusid`=?';
	const FIELD_POSTATUSID=1469731763;
	const FIELD_POSTATUS=908790936;
	const FIELD_PRESENCE=-1125284126;
	const FIELD_PICKLIST_VALUEID=-1304278733;
	private static $PRIMARY_KEYS=array(self::FIELD_POSTATUSID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_POSTATUSID);
	private static $FIELD_NAMES=array(
		self::FIELD_POSTATUSID=>'postatusid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_POSTATUSID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_POSTATUSID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_POSTATUS=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	/**
	 * set value for postatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $poStatusId
	 * @return VTigerPoStatus
	 */
	public function &setPoStatusId($poStatusId) {
		$this->notifyChanged(self::FIELD_POSTATUSID,$this->poStatusId,$poStatusId);
		$this->poStatusId=$poStatusId;
		return $this;
	}

	/**
	 * get value for postatusid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getPoStatusId() {
	...
	public function toArray() {
		return array(
			self::FIELD_POSTATUSID=>$this->getPoStatusId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerPoStatus instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerPoStatus $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerPoStatus[]
	 */
	public static function findByExample(PDO $db,VTigerPoStatus $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerPoStatus instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_postatus`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerPoStatus instances
	 *
	...
	/**
	 * returns the result as an array of VTigerPoStatus instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerPoStatus[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerPoStatus instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_postatus`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setPoStatusId($result['postatusid']);
	 ...
	 * @return VTigerPoStatus
	 */
	public static function findById(PDO $db,$poStatusId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$poStatusId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getPoStatusId());
		$stmt->bindValue(2,$this->getPoStatus());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getPoStatusId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getPoStatus());
		...
		if (false!==$lastInsertId) {
			$this->setPoStatusId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getPoStatusId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerPoStatus');
	}

	/**
	 * get single VTigerPoStatus instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerPoStatus from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerPoStatus') as $node) {
