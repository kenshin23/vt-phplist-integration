 ...
 * @package entity
 */
class VTigerMailScannerIds extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerMailScannerIds';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_mailscanner_ids';
	const SQL_INSERT='INSERT INTO `vtiger_mailscanner_ids` (`scannerid`,`messageid`,`crmid`) VALUES (?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_mailscanner_ids` (`scannerid`,`messageid`,`crmid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_mailscanner_ids` SET `scannerid`=?,`messageid`=?,`crmid`=? WHERE `scannerid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_mailscanner_ids` WHERE `scannerid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_mailscanner_ids` WHERE `scannerid`=?';
	const FIELD_SCANNERID=-1610936491;
	const FIELD_MESSAGEID=-1239106786;
	const FIELD_CRMID=1041850677;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerMailScannerIds instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerMailScannerIds $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerMailScannerIds[]
	 */
	public static function findByExample(PDO $db,VTigerMailScannerIds $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerMailScannerIds instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_mailscanner_ids`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerMailScannerIds instances
	 *
	...
	/**
	 * returns the result as an array of VTigerMailScannerIds instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerMailScannerIds[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerMailScannerIds instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_mailscanner_ids`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(4,$this->getScannerId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerMailScannerIds';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerMailScannerIds');
	}

	/**
	 * get single VTigerMailScannerIds instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerMailScannerIds from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerMailScannerIds') as $node) {
