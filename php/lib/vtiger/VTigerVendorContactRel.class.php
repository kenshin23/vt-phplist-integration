 ...
 * @package entity
 */
class VTigerVendorContactRel extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerVendorContactRel';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_vendorcontactrel';
	const SQL_INSERT='INSERT INTO `vtiger_vendorcontactrel` (`vendorid`,`contactid`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_vendorcontactrel` (`vendorid`,`contactid`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `vtiger_vendorcontactrel` SET `vendorid`=?,`contactid`=? WHERE `vendorid`=? AND `contactid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_vendorcontactrel` WHERE `vendorid`=? AND `contactid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_vendorcontactrel` WHERE `vendorid`=? AND `contactid`=?';
	const FIELD_VENDORID=1055033658;
	const FIELD_CONTACTID=-978863292;
	private static $PRIMARY_KEYS=array(self::FIELD_VENDORID,self::FIELD_CONTACTID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerVendorContactRel instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerVendorContactRel $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerVendorContactRel[]
	 */
	public static function findByExample(PDO $db,VTigerVendorContactRel $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerVendorContactRel instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_vendorcontactrel`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerVendorContactRel instances
	 *
	...
	/**
	 * returns the result as an array of VTigerVendorContactRel instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerVendorContactRel[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerVendorContactRel instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_vendorcontactrel`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return VTigerVendorContactRel
	 */
	public static function findById(PDO $db,$vendorId,$contactId) {
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(3,$this->getVendorId());
		$stmt->bindValue(4,$this->getContactId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	/**
	 * Fetch VTigerVendor which references this VTigerVendorContactRel. Will return null in case reference is invalid.
	 * `vtiger_vendorcontactrel`.`vendorid` -> `vtiger_vendor`.`vendorid`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return VTigerVendor
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='VTigerVendorContactRel';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerVendorContactRel');
	}

	/**
	 * get single VTigerVendorContactRel instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerVendorContactRel from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerVendorContactRel') as $node) {
