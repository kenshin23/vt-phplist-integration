 ...
 * @package entity
 */
class VTigerUsageUnit extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='VTigerUsageUnit';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='vtiger_usageunit';
	const SQL_INSERT='INSERT INTO `vtiger_usageunit` (`usageunitid`,`usageunit`,`presence`,`picklist_valueid`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `vtiger_usageunit` (`usageunit`,`presence`,`picklist_valueid`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `vtiger_usageunit` SET `usageunitid`=?,`usageunit`=?,`presence`=?,`picklist_valueid`=? WHERE `usageunitid`=?';
	const SQL_SELECT_PK='SELECT * FROM `vtiger_usageunit` WHERE `usageunitid`=?';
	const SQL_DELETE_PK='DELETE FROM `vtiger_usageunit` WHERE `usageunitid`=?';
	const FIELD_USAGEUNITID=819399425;
	const FIELD_USAGEUNIT=371801958;
	const FIELD_PRESENCE=1481269274;
	const FIELD_PICKLIST_VALUEID=-1565627797;
	private static $PRIMARY_KEYS=array(self::FIELD_USAGEUNITID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_USAGEUNITID);
	private static $FIELD_NAMES=array(
		self::FIELD_USAGEUNITID=>'usageunitid',
	...
	private static $PROPERTY_TYPES=array(
		self::FIELD_USAGEUNITID=>Db2PhpEntity::PHP_TYPE_INT,
	...
	private static $FIELD_TYPES=array(
		self::FIELD_USAGEUNITID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_USAGEUNIT=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,200,0,false),
	...
	private static $DEFAULT_VALUES=array(
		self::FIELD_USAGEUNITID=>null,
		self::FIELD_USAGEUNIT=>'',
	...
	/**
	 * set value for usageunitid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @param mixed $usageUnitId
	 * @return VTigerUsageUnit
	 */
	public function &setUsageUnitId($usageUnitId) {
		$this->notifyChanged(self::FIELD_USAGEUNITID,$this->usageUnitId,$usageUnitId);
		$this->usageUnitId=$usageUnitId;
		return $this;
	}

	/**
	 * get value for usageunitid 
	 *
	 * type:INT,size:10,default:null,primary,unique,autoincrement
	 *
	 * @return mixed
	 */
	public function getUsageUnitId() {
	...
	public function toArray() {
		return array(
			self::FIELD_USAGEUNITID=>$this->getUsageUnitId(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of VTigerUsageUnit instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param VTigerUsageUnit $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return VTigerUsageUnit[]
	 */
	public static function findByExample(PDO $db,VTigerUsageUnit $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of VTigerUsageUnit instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `vtiger_usageunit`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of VTigerUsageUnit instances
	 *
	...
	/**
	 * returns the result as an array of VTigerUsageUnit instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return VTigerUsageUnit[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of VTigerUsageUnit instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `vtiger_usageunit`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	public function assignByHash($result) {
		$this->setUsageUnitId($result['usageunitid']);
	 ...
	 * @return VTigerUsageUnit
	 */
	public static function findById(PDO $db,$usageUnitId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$usageUnitId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		$stmt->bindValue(1,$this->getUsageUnitId());
		$stmt->bindValue(2,$this->getUsageUnit());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getUsageUnitId()) {
			$stmt=self::prepareStatement($db,self::SQL_INSERT_AUTOINCREMENT);
			$stmt->bindValue(1,$this->getUsageUnit());
		...
		if (false!==$lastInsertId) {
			$this->setUsageUnitId($lastInsertId);
		}
		$stmt->closeCursor();
		$this->notifyPristine();
		return $affected;
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getUsageUnitId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'VTigerUsageUnit');
	}

	/**
	 * get single VTigerUsageUnit instance from a DOMElement
	 *
	...
	/**
	 * get all instances of VTigerUsageUnit from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('VTigerUsageUnit') as $node) {
