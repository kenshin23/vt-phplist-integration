 ...
 * @package entity
 */
class PhPListTemplate extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListTemplate';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_template';
	const SQL_INSERT='INSERT INTO `phplist_template` (`id`,`title`,`template`,`listorder`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_template` (`title`,`template`,`listorder`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_template` SET `id`=?,`title`=?,`template`=?,`listorder`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_template` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_template` WHERE `id`=?';
	const FIELD_ID=-217139610;
	const FIELD_TITLE=-574951123;
	const FIELD_TEMPLATE=509219205;
	const FIELD_LISTORDER=-290200123;
	...
	private static $PROPERTY_TYPES=array(
		...
		self::FIELD_TEMPLATE=>Db2PhpEntity::PHP_TYPE_STRING,
	...
	private static $FIELD_TYPES=array(
		...
		self::FIELD_TEMPLATE=>array(Db2PhpEntity::JDBC_TYPE_LONGVARBINARY,2147483647,0,true),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListTemplate instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListTemplate $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListTemplate[]
	 */
	public static function findByExample(PDO $db,PhPListTemplate $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListTemplate instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_template`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListTemplate instances
	 *
	...
	/**
	 * returns the result as an array of PhPListTemplate instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListTemplate[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListTemplate instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_template`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getTemplate());
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListTemplate');
	}

	/**
	 * get single PhPListTemplate instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListTemplate from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListTemplate') as $node) {
