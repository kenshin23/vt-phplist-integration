 ...
 * @package entity
 */
class PhPListLinkTrackUserClick extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListLinkTrackUserClick';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_linktrack_userclick';
	const SQL_INSERT='INSERT INTO `phplist_linktrack_userclick` (`linkid`,`userid`,`messageid`,`name`,`data`,`date`) VALUES (?,?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_linktrack_userclick` (`linkid`,`userid`,`messageid`,`name`,`data`,`date`) VALUES (?,?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_linktrack_userclick` SET `linkid`=?,`userid`=?,`messageid`=?,`name`=?,`data`=?,`date`=? WHERE `linkid`=? AND `userid`=? AND `messageid`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_linktrack_userclick` WHERE `linkid`=? AND `userid`=? AND `messageid`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_linktrack_userclick` WHERE `linkid`=? AND `userid`=? AND `messageid`=?';
	const FIELD_LINKID=-2066911811;
	const FIELD_USERID=-1800275634;
	const FIELD_MESSAGEID=1751985978;
	const FIELD_NAME=1687284659;
	const FIELD_DATA=1686986962;
	const FIELD_DATE=1686986966;
	private static $PRIMARY_KEYS=array(self::FIELD_LINKID,self::FIELD_USERID,self::FIELD_MESSAGEID);
	 ...
	 *
	 * @param mixed $linkId
	 * @return PhPListLinkTrackUserClick
	 */
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListLinkTrackUserClick instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListLinkTrackUserClick $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListLinkTrackUserClick[]
	 */
	public static function findByExample(PDO $db,PhPListLinkTrackUserClick $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListLinkTrackUserClick instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_linktrack_userclick`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListLinkTrackUserClick instances
	 *
	...
	/**
	 * returns the result as an array of PhPListLinkTrackUserClick instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListLinkTrackUserClick[]
	 */
	public static function fromExecutedStatement(PDOStatement $stmt) {
		...
		while($result=$stmt->fetch(PDO::FETCH_ASSOC)) {
			$o=new PhPListLinkTrackUserClick();
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListLinkTrackUserClick instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_linktrack_userclick`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return PhPListLinkTrackUserClick
	 */
	public static function findById(PDO $db,$linkId,$userId,$messageId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$linkId);
		$stmt->bindValue(2,$userId);
		$stmt->bindValue(3,$messageId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getMessageId());
		$stmt->bindValue(4,$this->getName());
		$stmt->bindValue(5,$this->getData());
		$stmt->bindValue(6,$this->getDate());
	}
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(7,$this->getLinkId());
		$stmt->bindValue(8,$this->getUserId());
		$stmt->bindValue(9,$this->getMessageId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='PhPListLinkTrackUserClick';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListLinkTrackUserClick');
	}

	/**
	 * get single PhPListLinkTrackUserClick instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListLinkTrackUserClick from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListLinkTrackUserClick') as $node) {
