 ...
 * @package entity
 */
class PhPListUserBlacklist extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListUserBlacklist';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_user_blacklist';
	const SQL_INSERT='INSERT INTO `phplist_user_blacklist` (`email`,`added`) VALUES (?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_user_blacklist` (`email`,`added`) VALUES (?,?)';
	const SQL_UPDATE='UPDATE `phplist_user_blacklist` SET `email`=?,`added`=? WHERE `email`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_user_blacklist` WHERE `email`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_user_blacklist` WHERE `email`=?';
	const FIELD_EMAIL=322609568;
	const FIELD_ADDED=318650116;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListUserBlacklist instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListUserBlacklist $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListUserBlacklist[]
	 */
	public static function findByExample(PDO $db,PhPListUserBlacklist $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListUserBlacklist instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_user_blacklist`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListUserBlacklist instances
	 *
	...
	/**
	 * returns the result as an array of PhPListUserBlacklist instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListUserBlacklist[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListUserBlacklist instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_user_blacklist`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(2,$this->getAdded());
	}
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='PhPListUserBlacklist';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListUserBlacklist');
	}

	/**
	 * get single PhPListUserBlacklist instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListUserBlacklist from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListUserBlacklist') as $node) {
