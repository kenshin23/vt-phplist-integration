 ...
 * @package entity
 */
class PhPListUrlCache extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListUrlCache';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_urlcache';
	const SQL_INSERT='INSERT INTO `phplist_urlcache` (`id`,`url`,`lastmodified`,`added`,`content`) VALUES (?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_urlcache` (`url`,`lastmodified`,`added`,`content`) VALUES (?,?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_urlcache` SET `id`=?,`url`=?,`lastmodified`=?,`added`=?,`content`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_urlcache` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_urlcache` WHERE `id`=?';
	const FIELD_ID=-774472499;
	const FIELD_URL=1761168381;
	const FIELD_LASTMODIFIED=533043345;
	const FIELD_ADDED=246807566;
	const FIELD_CONTENT=-1236483065;
	...
	private static $PROPERTY_TYPES=array(
		...
		self::FIELD_LASTMODIFIED=>Db2PhpEntity::PHP_TYPE_INT,
		self::FIELD_ADDED=>Db2PhpEntity::PHP_TYPE_STRING,
		self::FIELD_CONTENT=>Db2PhpEntity::PHP_TYPE_STRING);
	private static $FIELD_TYPES=array(
		self::FIELD_ID=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,false),
		self::FIELD_URL=>array(Db2PhpEntity::JDBC_TYPE_VARCHAR,255,0,false),
		self::FIELD_LASTMODIFIED=>array(Db2PhpEntity::JDBC_TYPE_INTEGER,10,0,true),
		self::FIELD_ADDED=>array(Db2PhpEntity::JDBC_TYPE_TIMESTAMP,19,0,true),
		self::FIELD_CONTENT=>array(Db2PhpEntity::JDBC_TYPE_LONGVARCHAR,16777215,0,true));
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListUrlCache instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListUrlCache $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListUrlCache[]
	 */
	public static function findByExample(PDO $db,PhPListUrlCache $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListUrlCache instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_urlcache`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListUrlCache instances
	 *
	...
	/**
	 * returns the result as an array of PhPListUrlCache instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListUrlCache[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListUrlCache instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_urlcache`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getLastModified());
		$stmt->bindValue(4,$this->getAdded());
		$stmt->bindValue(5,$this->getContent());
	}
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getId()) {
			...
			$stmt->bindValue(1,$this->getUrl());
			$stmt->bindValue(2,$this->getLastModified());
			$stmt->bindValue(3,$this->getAdded());
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListUrlCache');
	}

	/**
	 * get single PhPListUrlCache instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListUrlCache from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListUrlCache') as $node) {
