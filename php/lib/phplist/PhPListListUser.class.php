 ...
 * @package entity
 */
class PhPListListUser extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListListUser';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_listuser';
	const SQL_INSERT='INSERT INTO `phplist_listuser` (`userid`,`listid`,`entered`,`modified`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_listuser` (`userid`,`listid`,`entered`,`modified`) VALUES (?,?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_listuser` SET `userid`=?,`listid`=?,`entered`=?,`modified`=? WHERE `userid`=? AND `listid`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_listuser` WHERE `userid`=? AND `listid`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_listuser` WHERE `userid`=? AND `listid`=?';
	const FIELD_USERID=1240715810;
	const FIELD_LISTID=974237237;
	const FIELD_ENTERED=-1637344005;
	const FIELD_MODIFIED=-2022665275;
	private static $PRIMARY_KEYS=array(self::FIELD_USERID,self::FIELD_LISTID);
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListListUser instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListListUser $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListListUser[]
	 */
	public static function findByExample(PDO $db,PhPListListUser $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListListUser instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_listuser`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListListUser instances
	 *
	...
	/**
	 * returns the result as an array of PhPListListUser instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListListUser[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListListUser instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_listuser`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return PhPListListUser
	 */
	public static function findById(PDO $db,$userId,$listId) {
		$stmt=self::prepareStatement($db,self::SQL_SELECT_PK);
		$stmt->bindValue(1,$userId);
		$stmt->bindValue(2,$listId);
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(2,$this->getListId());
		$stmt->bindValue(3,$this->getEntered());
		$stmt->bindValue(4,$this->getModified());
	}
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(5,$this->getUserId());
		$stmt->bindValue(6,$this->getListId());
		$affected=$stmt->execute();
		if (false===$affected) {
			$stmt->closeCursor();
			throw new Exception($stmt->errorCode() . ':' . var_export($stmt->errorInfo(), true), 0);
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListListUser');
	}

	/**
	 * get single PhPListListUser instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListListUser from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListListUser') as $node) {
