 ...
 * @package entity
 */
class PhPListUserMessage extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListUserMessage';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_usermessage';
	const SQL_INSERT='INSERT INTO `phplist_usermessage` (`messageid`,`userid`,`entered`,`viewed`,`status`) VALUES (?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_usermessage` (`messageid`,`userid`,`entered`,`viewed`,`status`) VALUES (?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_usermessage` SET `messageid`=?,`userid`=?,`entered`=?,`viewed`=?,`status`=? WHERE `messageid`=? AND `userid`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_usermessage` WHERE `messageid`=? AND `userid`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_usermessage` WHERE `messageid`=? AND `userid`=?';
	const FIELD_MESSAGEID=657847399;
	const FIELD_USERID=1645349697;
	const FIELD_ENTERED=-1978595396;
	const FIELD_VIEWED=1664748319;
	const FIELD_STATUS=1588898061;
	private static $PRIMARY_KEYS=array(self::FIELD_MESSAGEID,self::FIELD_USERID);
	...
	private static $PROPERTY_TYPES=array(
		...
		self::FIELD_VIEWED=>Db2PhpEntity::PHP_TYPE_STRING,
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListUserMessage instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListUserMessage $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListUserMessage[]
	 */
	public static function findByExample(PDO $db,PhPListUserMessage $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListUserMessage instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_usermessage`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListUserMessage instances
	 *
	...
	/**
	 * returns the result as an array of PhPListUserMessage instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListUserMessage[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListUserMessage instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_usermessage`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	 ...
	 * @return PhPListUserMessage
	 */
	public static function findById(PDO $db,$messageId,$userId) {
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(4,$this->getViewed());
	...
	public function updateToDatabase(PDO $db) {
		...
		$this->bindValues($stmt);
		$stmt->bindValue(6,$this->getMessageId());
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListUserMessage');
	}

	/**
	 * get single PhPListUserMessage instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListUserMessage from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListUserMessage') as $node) {
