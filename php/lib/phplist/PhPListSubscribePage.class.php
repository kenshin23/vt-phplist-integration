 ...
 * @package entity
 */
class PhPListSubscribePage extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListSubscribePage';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_subscribepage';
	const SQL_INSERT='INSERT INTO `phplist_subscribepage` (`id`,`title`,`active`,`owner`) VALUES (?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_subscribepage` (`title`,`active`,`owner`) VALUES (?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_subscribepage` SET `id`=?,`title`=?,`active`=?,`owner`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_subscribepage` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_subscribepage` WHERE `id`=?';
	const FIELD_ID=-11845799;
	const FIELD_TITLE=-700457126;
	const FIELD_ACTIVE=-788831676;
	const FIELD_OWNER=-704663627;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListSubscribePage instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListSubscribePage $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListSubscribePage[]
	 */
	public static function findByExample(PDO $db,PhPListSubscribePage $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListSubscribePage instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_subscribepage`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListSubscribePage instances
	 *
	...
	/**
	 * returns the result as an array of PhPListSubscribePage instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListSubscribePage[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListSubscribePage instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_subscribepage`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getActive());
		$stmt->bindValue(4,$this->getOwner());
	}
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getId()) {
			...
			$stmt->bindValue(2,$this->getActive());
			$stmt->bindValue(3,$this->getOwner());
		} else {
	...
	public static function toEzcPersistentObjectDefinition() {
		...
		$def->class='PhPListSubscribePage';
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListSubscribePage');
	}

	/**
	 * get single PhPListSubscribePage instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListSubscribePage from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListSubscribePage') as $node) {
