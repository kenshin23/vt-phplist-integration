 ...
 * @package entity
 */
class PhPListBounce extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PhPListBounce';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='phplist_bounce';
	const SQL_INSERT='INSERT INTO `phplist_bounce` (`id`,`date`,`header`,`data`,`status`,`comment`) VALUES (?,?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `phplist_bounce` (`date`,`header`,`data`,`status`,`comment`) VALUES (?,?,?,?,?)';
	const SQL_UPDATE='UPDATE `phplist_bounce` SET `id`=?,`date`=?,`header`=?,`data`=?,`status`=?,`comment`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `phplist_bounce` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `phplist_bounce` WHERE `id`=?';
	const FIELD_ID=-1443589288;
	const FIELD_DATE=-15017301;
	const FIELD_HEADER=-1429077430;
	const FIELD_DATA=-15017305;
	const FIELD_STATUS=-1100288081;
	const FIELD_COMMENT=-1196636606;
	private static $PRIMARY_KEYS=array(self::FIELD_ID);
	private static $AUTOINCREMENT_FIELDS=array(self::FIELD_ID);
	private static $FIELD_NAMES=array(
		self::FIELD_ID=>'id',
		self::FIELD_DATE=>'date',
		self::FIELD_HEADER=>'header',
	...
	private static $PROPERTY_TYPES=array(
		...
		self::FIELD_HEADER=>Db2PhpEntity::PHP_TYPE_STRING,
	...
	private static $FIELD_TYPES=array(
		...
		self::FIELD_HEADER=>array(Db2PhpEntity::JDBC_TYPE_LONGVARCHAR,65535,0,true),
		self::FIELD_DATA=>array(Db2PhpEntity::JDBC_TYPE_LONGVARBINARY,65535,0,true),
	...
	public function &setHeader($header) {
		$this->notifyChanged(self::FIELD_HEADER,$this->header,$header);
		$this->header=$header;
		return $this;
	}
	...
	public function toArray() {
		return array(
			...
			self::FIELD_HEADER=>$this->getHeader(),
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PhPListBounce instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PhPListBounce $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PhPListBounce[]
	 */
	public static function findByExample(PDO $db,PhPListBounce $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PhPListBounce instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	public static function findByFilter(PDO $db, $filter, $and=true, $sort=null) {
		...
		$sql='SELECT * FROM `phplist_bounce`'
	...
	/**
	 * Will execute the passed statement and return the result as an array of PhPListBounce instances
	 *
	...
	/**
	 * returns the result as an array of PhPListBounce instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PhPListBounce[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PhPListBounce instances.
	 *
	...
	public static function deleteByFilter(PDO $db, $filter, $and=true) {
		...
		$sql='DELETE FROM `phplist_bounce`'
		. self::buildSqlWhere($filter, $and, false, true);
		$stmt=self::prepareStatement($db, $sql);
		self::bindValuesForFilter($stmt, $filter);
		$affected=$stmt->execute();
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getHeader());
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getId()) {
			...
			$stmt->bindValue(2,$this->getHeader());
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PhPListBounce');
	}

	/**
	 * get single PhPListBounce instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PhPListBounce from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PhPListBounce') as $node) {
